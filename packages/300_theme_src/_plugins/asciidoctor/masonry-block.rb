...
Asciidoctor::Extensions.register do
  ...
  class MasonryBlockMacro < Extensions::BlockMacroProcessor
    ...
    def process parent, target, attrs
      title_html  = (attrs.has_key? 'title') ? %(<div class="masonry-title speak2me-ignore notranslate">#{attrs['title']}</div>\n) : nil
#     html = %(#{title_html} <div id="#{target}_parent" class="container g-0 masonry-parent #{attrs['role']}"></div>)
      html = %(#{title_html} <div id="#{target}_parent" class="masonry-parent speak2me-ignore #{attrs['role']}"></div>)

...
Asciidoctor::Extensions.register do
  ...
  class ImageBlockMacro < Extensions::BlockMacroProcessor
    ...
    def process parent, target, attrs
      title_html  = (attrs.has_key? 'title') ? %(<div class="gallery-title speak2me-ignore notranslate">#{attrs['title']}</div>\n) : nil
      html = %(#{title_html} <div id="#{target}" class="speak2me-ignore #{attrs['role']}"></div>)

...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/amplitude.js (47)
...
//var playerVolumeSliderStep            = {{amplitude_default.player.volume_slider.slider_step}};
  ...
  return {
    ...
    init: (options) => {
      ...
      {% comment %} claude - Fix Amplitude YAML data processing #1
      amplitudePlayers now reads the CONTROL settings (user layer + the
      per-player array 'players'). The former source amplitude_player was
      NO LONGER ASSIGNED after the config files were renamed for v46
      (amplitude_player.yml -> amplitude_control.yml), so the object was
      always empty.
      Original (deprecated, preserved for reference):
      ...
      // claude - Fix Amplitude YAML data processing #1
      // expose the global merged options (defaults <- user settings) and
      // the per-instance options cache for module|plugin (ytp) use
    ...
    // setState()
    // claude - Fix Amplitude YAML data processing #1
    // NOTE: setState() follows BELOW the two new methods inserted here
    // (_deepMerge, getInstanceOptions); label kept for reference.
    // -------------------------------------------------------------------------
    // claude - Fix Amplitude YAML data processing #1

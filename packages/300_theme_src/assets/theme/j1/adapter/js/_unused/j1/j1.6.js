...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/core/js/j1.js (6)
...
/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/j1.js (6)
...
var j1 = ((options) => {
  ...
  return {
    ...
    init: (options) => {
      ...
      if (user_session.mode === 'app') {
        ...
        .catch((error) => {
          ...
          setTimeout(() => {
            ...
            var blocksReadyTimeout1;
            var footerBlockTimeout1;
            ...
            var dependencies_met_page_ready = setInterval (() => {
              ...
              if (j1CoreFinished && pageVisible) {
                ...
                if (banner_blocks || panel_blocks) {
                  var dependencies_met_blocks_ready = setInterval (() => {
                    ...
                    if (banner_state == 'success' && panel_state == 'success' && footer_state == 'success') {
                      ...
                      if (blocksReadyTimeout1) {
                        clearTimeout(blocksReadyTimeout1);
                        blocksReadyTimeout1 = null;
                      }
                    }
                  ...
                  // initFooter completing successfully. A 404, a network error,
                  // or any single block remaining in 'pending' state would
                  // otherwise pin a 10ms interval for the lifetime of the tab
                  // — and visually leave the page hidden behind #no_flicker
                  // because the .show() that reveals the content lives inside
                  // the success branch. Cap at 30s, log a warning that names
                  // which block stalled, and reveal the content anyway so
                  // the page is at least usable in the failure case.
                  //
                  blocksReadyTimeout1 = setTimeout(function () {
                    if (dependencies_met_blocks_ready) {
                      clearInterval(dependencies_met_blocks_ready);
                      logger.warn('j1 init: block resources did not load within 30s (banner=' + banner_state + ', panel=' + panel_state + ', footer=' + footer_state + '); revealing content anyway');
                      $('#no_flicker').show();
                      $('.{{footer}}').show();
                    }
                  }, 30000);
                  ...
                  var dependencies_met_footer_block_ready = setInterval (() => {
                    ...
                    if (footer_state == 'success') {
                      ...
                      if (footerBlockTimeout1) {
                        clearTimeout(footerBlockTimeout1);
                        footerBlockTimeout1 = null;
                      }
                    } // END if footer
                  }, 10);
                  ...
                  // #no_flicker forever and the 10ms poller pins CPU.
                  // Cap at 30s and reveal content anyway.
                  //
                  footerBlockTimeout1 = setTimeout(function () {
                    if (dependencies_met_footer_block_ready) {
                      clearInterval(dependencies_met_footer_block_ready);
                      logger.warn('j1 init: footer did not load within 30s (footer=' + footer_state + '); revealing content anyway');
                      $('#no_flicker').show();
                      $('.{{footer}}').show();
                    }
                  }, 30000);
                ...
                if (pageReadyTimeout1) {
                  clearTimeout(pageReadyTimeout1);
                  pageReadyTimeout1 = null;
                }
              } // END if pageVisible
            ...
            // aborted navigation), this 10ms interval ran for the lifetime
            // of the tab. Cap at 30s and log a warning so the failure mode
            // is visible in the console.
            //
            pageReadyTimeout1 = setTimeout(function () {
              if (dependencies_met_page_ready) {
                clearInterval(dependencies_met_page_ready);
                logger.warn('j1 init aborted: page-ready conditions not met within 30s');
              }
            }, 30000);
      ...
      var blocksReadyTimeout2;
      var footerBlockTimeout2;
      ...
      var dependencies_met_page_ready = setInterval (() => {
        ...
        if (pageVisible) {
          ...
          if (banner_blocks || panel_blocks) {
            var dependencies_met_blocks_ready = setInterval (() => {
              ...
              if (banner_state == 'success' && panel_state == 'success' && footer_state == 'success') {
                ...
                if (blocksReadyTimeout2) {
                  clearTimeout(blocksReadyTimeout2);
                  blocksReadyTimeout2 = null;
                }
              }
            }, 10);
            ...
            // Cap at 30s, log a warning, and reveal content anyway so the
            // page is at least usable in the failure case.
            //
            blocksReadyTimeout2 = setTimeout(function () {
              if (dependencies_met_blocks_ready) {
                clearInterval(dependencies_met_blocks_ready);
                logger.warn('j1 init: block resources did not load within 30s (banner=' + banner_state + ', panel=' + panel_state + ', footer=' + footer_state + '); revealing content anyway');
                $('#no_flicker').show();
            ...
            if (pageReadyTimeout2) {
              clearTimeout(pageReadyTimeout2);
              pageReadyTimeout2 = null;
            }
            // END if  banner_blocks || panel_blocks
          } else {
            // process pages w/o banners or panels
            ...
            var dependencies_met_footer_block_ready = setInterval (() => {
              ...
              if (footer_state == 'success') {
                ...
                if (footerBlockTimeout2) {
                  clearTimeout(footerBlockTimeout2);
                  footerBlockTimeout2 = null;
                }
              } // END  if footer
            ...
            //
            footerBlockTimeout2 = setTimeout(function () {
              if (dependencies_met_footer_block_ready) {
                clearInterval(dependencies_met_footer_block_ready);
                logger.warn('j1 init: footer did not load within 30s (footer=' + footer_state + '); revealing content anyway');
                $('#no_flicker').show();
      ...
      // lifetime of the tab. Cap at 30s and log a warning.
      //
      pageReadyTimeout2 = setTimeout(function () {
        if (dependencies_met_page_ready) {
          clearInterval(dependencies_met_page_ready);
          logger.warn('j1 init aborted: page-ready conditions not met within 30s');
        }
    ...
    finalizePage: (options) => {
      ...
      if (j1.appDetected()) {
        ...
        .then(function(data) {
          ...
          var dependencies_met_page_ready = setInterval (() => {
            ...
            if (j1CoreFinished && pageVisible) {
              ...
              if (finalizeScrollTimeout1) {
                clearTimeout(finalizeScrollTimeout1);
                finalizeScrollTimeout1 = null;
              }
            }
          }, 10);
          ...
          // of finalizePage being called, but if the boot sequence stalls
          // — or if this branch is reached on a page where #no_flicker
          // was already removed/never inserted — the 10ms interval would
          // run for the lifetime of the tab. Cap at 30s and log.
          //
          finalizeScrollTimeout1 = setTimeout(function () {
            if (dependencies_met_page_ready) {
              clearInterval(dependencies_met_page_ready);
              logger.warn('j1 finalizePage (app): scroll-anchor poller timed out at 5s');
            }
          }, 5000);
        ...
        var dependencies_met_page_ready = setInterval (() => {
          ...
          if (j1CoreFinished && pageVisible) {
            ...
            if (finalizeScrollTimeout2) {
              clearTimeout(finalizeScrollTimeout2);
              finalizeScrollTimeout2 = null;
            }
          } // END if pageVisible
        }, 10); // END dependencies_met_page_ready
        ...
        //
        finalizeScrollTimeout2 = setTimeout(function () {
          if (dependencies_met_page_ready) {
            clearInterval(dependencies_met_page_ready);
            logger.warn('j1 finalizePage (web): scroll-anchor poller timed out at 5s');
          }
        }, 5000);
    ...
    resolveMacros: (user_data) => {
        ...
        if ($('#sidebar_mmenu').length) {
          if (Object.keys(user_data).length) {
            ...
            if (sidebarLoadedTimeout) {
              clearTimeout(sidebarLoadedTimeout);
              sidebarLoadedTimeout = null;
            }
            return true;
          } else {
      ...
      // the mmenu (login screens, embedded views, error pages) the
      // selector never resolves and the 10ms interval would otherwise
      // run for the lifetime of the tab. Cap at 30s and log a warning.
      //
      sidebarLoadedTimeout = setTimeout(function () {
        if (sidebarLoaded) {
          clearInterval(sidebarLoaded);
          logger.warn('j1 resolveMacros: #sidebar_mmenu not found within 30s');
        }
      }, 30000);
    }, // END resolveMacros
    ...
    updateMacros: (user_data) => {
      ...
      sidebarLoadedTimeout = setTimeout(function () {
        if (sidebarLoaded) {
          clearInterval(sidebarLoaded);
          logger.warn('j1 updateMacros: #sidebar_mmenu not found within 5s');
        }
      }, 5000);
    }, // END updateMacros
...
//              var scrollDuration          = 300;
      ...
      // or the page monitor never classified the page, this 10ms interval
      // would run for the lifetime of the tab. Cap at 30s and log a
      // warning. Note: the scroll-to-anchor effect is purely cosmetic
      // (URL hash navigation), so failing silently is acceptable — the
      // page is otherwise functional.
      //
      scrollAnchorTimeout = setTimeout(function () {
        if (dependencies_met_page_displayed) {
          clearInterval(dependencies_met_page_displayed);
          logger.warn('j1 scrollToAnchor: page-displayed poller timed out at 30s (pageType=' + j1['pageMonitor'].pageType + ')');
        }
      }, 30000);
    }, // END scrollToAnchor

...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theFdames/j1/adapter/js/advertising.js (1)
...
j1.adapter.advertising = ((j1, window) => {
  ...
  // boot sequence, an aborted navigation, an extension hiding #content,
  // or no .adsbygoogle elements ever appearing for ad_initializer),
  // these 10ms intervals ran for the lifetime of the tab. Cap each at
  // 30s and log a warning so the failure mode is visible in the console
  // instead of silently burning CPU.
  //
  var dependenciesTimeout;
  var adInitializerTimeout;
  ...
  return {
    ...
    init: (options) => {
      ...
      dependenciesTimeout = setTimeout(function () {
        if (dependencies_met_page_ready) {
          clearInterval(dependencies_met_page_ready);
          logger.warn('advertising init aborted: page-ready conditions not met within 30s');
        }
      }, 30000);
    }, // END init
    ...
    ad_initializer: () => {
      ...
      var dependencies_met_page_visible = setInterval (() => {
        ...
        if (j1CoreFinished && pageVisible && ads_found) {
          ...
          if (adInitializerTimeout) {
            clearTimeout(adInitializerTimeout);
            adInitializerTimeout = null;
          }
      ...
      //
      adInitializerTimeout = setTimeout(function () {
        if (dependencies_met_page_visible) {
          clearInterval(dependencies_met_page_visible);
          logger.warn('advertising ad_initializer aborted: page-visible/ads conditions not met within 5s');
        }
      }, 5000);
    }, // END init

...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/iframer.js (1)
    ...
    init: (options) => {
      ...
      var dependencies_met_page_ready = setInterval (() => {
        ...
        if (j1CoreFinished && pageVisible) {
          ...
          // ("module initializing time: NaNms") in every log session.
          //
      ...
      dependenciesTimeout = setTimeout(function () {
        if (dependencies_met_page_ready) {
          clearInterval(dependencies_met_page_ready);
          logger.warn('iframer init aborted: page-ready conditions not met within 5s');
        }
      }, 5000);
    }, // END init
        ...
        logger.info('found iframe on id: ' + '{{iframe_id}}');
          ...
          load_dependencies['dependencies_met_html_loaded_{{iframe_id}}'] = setInterval (() => {
            ...
            if (xhrLoadState === 'success') {
              ...
              if ('{{iframe.inject_contentWindowScript}}' == 'true') {
                setTimeout(() => {
                  ...
                  contentWindowScript.src   = origin + '/assets/theme/j1/modules/iframeResizer/js/client/iframeResizer.contentWindow.min.js';
            ...
            if (load_dependencies['html_loaded_timeout_{{iframe_id}}']) {
              clearTimeout(load_dependencies['html_loaded_timeout_{{iframe_id}}']);
              load_dependencies['html_loaded_timeout_{{iframe_id}}'] = null;
            }
          ...
          // page would otherwise leave this 10ms interval running for the
          // lifetime of the tab — once per configured iframe. Cap at 30s
          // and log a warning that names the iframe id so the failing
          // fetch is debuggable.
          //
          load_dependencies['html_loaded_timeout_{{iframe_id}}'] = setTimeout(function () {
            if (load_dependencies['dependencies_met_html_loaded_{{iframe_id}}']) {
              clearInterval(load_dependencies['dependencies_met_html_loaded_{{iframe_id}}']);
              logger.warn('iframer init aborted for {{iframe_id}}: HTML did not load within 30s (xhrDOMState=' + j1.xhrDOMState['#{{iframe_id}}_parent'] + ')');
            }
          }, 30000);

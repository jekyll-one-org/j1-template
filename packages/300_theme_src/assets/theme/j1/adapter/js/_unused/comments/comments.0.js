...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/comments.js (1)
...
j1.adapter.comments = ((j1, window) => {
  ...
  return {
    ...
    init: (options) => {
      ...
      // whichever interval was actually scheduled.
      //
      dependenciesTimeout = setTimeout(function () {
        if (typeof dependencies_met_page_ready !== 'undefined' && dependencies_met_page_ready) {
          clearInterval(dependencies_met_page_ready);
          if (logger) {
            logger.warn('comments init aborted: j1-finished condition not met within 5s');
          }
        }
      }, 5000);
    }, // END init

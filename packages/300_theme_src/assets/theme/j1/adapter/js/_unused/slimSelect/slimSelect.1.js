...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/slimSelect.js (1)
...
/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/slimSelect.js (1)
 # J1 Adapter for the slimSelect module
 #
 # Product/Info:
 # https://jekyll.one
 #
    ...
    init: (options) => {
      ...
      var dependency_met_page_ready = setInterval (() => {
        ...
        if (j1CoreFinished) {
          ...
          wrapper_dependencies['dependency_met_wrapper_ready_{{select.id}}'] = setInterval (() => {
            ...
            if (wrapperReady) {
              ...
              if (wrapper_timeouts['{{select.id}}']) {
                clearTimeout(wrapper_timeouts['{{select.id}}']);
                wrapper_timeouts['{{select.id}}'] = null;
              }
            } // END if wrapperReady
          ...
          wrapper_timeouts['{{select.id}}'] = setTimeout(() => {
            if (wrapper_dependencies['dependency_met_wrapper_ready_{{select.id}}']) {
              clearInterval(wrapper_dependencies['dependency_met_wrapper_ready_{{select.id}}']);
              logger.warn('slimSelect wrapper poll aborted for {{select.id}}: #{{select.wrapper_id}} not ready within 30s');
            }
          }, 30000);
      ...
      dependenciesTimeout = setTimeout(() => {
        if (dependency_met_page_ready) {
          clearInterval(dependency_met_page_ready);
          logger.warn('slimSelect init aborted: page-ready conditions not met within 5s');
        }
      }, 5000);
    }, // END init

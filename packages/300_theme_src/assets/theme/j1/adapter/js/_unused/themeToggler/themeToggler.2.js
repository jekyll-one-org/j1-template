...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/themeToggler.js (2)
...
j1.adapter.themeToggler = ((j1, window) => {
  ...
  return {
    ...
    init: (options) => {
      ...
      // `display: block`, j1.getState() never reached 'finished', or the
      // theme-toggler button (`#quickLinksThemeTogglerButton`) never appeared
      // in the DOM (e.g. layout disabled the quick-links bar, an aborted
      // navigation, an extension hiding #content), this 10ms interval ran
      // for the lifetime of the tab. Cap it at 30s and log a warning so the
      // failure mode is visible in the console instead of silently burning
      // CPU.
      //
      ...
      dependenciesTimeout = setTimeout(function () {
        if (dependencies_met_page_ready) {
          clearInterval(dependencies_met_page_ready);
          logger.warn('themeToggler init aborted: page-ready conditions not met within 5s');
        }
      }, 5000);
    }, // END init

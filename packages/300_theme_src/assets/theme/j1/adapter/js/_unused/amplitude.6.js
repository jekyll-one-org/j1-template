...
//        toJSON = JSON.stringify(songs, null, 2);
//        songs = toJSON;
//        songs = `${toJSON}`;
          ...
          toJSON    = JSON.stringify(playLists, null, 2);
          playLists = toJSON;
//        playLists = `${toJSON}`;
          ...
          var dependencies_met_api_initialized = setInterval (() => {
            if (playLists.length && playersUILoaded.state) {
...
//         } // END if playlist exists
        ...
        tmpPlaylist["{{playlist_name}}"]       = {};
//      tmpPlaylist["{{playlist_name}}"].title = '{{playlist_title}}'
        tmpPlaylist["{{playlist_name}}"].songs = [];
        tmpPlaylist["{{playlist_name}}"].songs = songs;

        playlist.push(tmpPlaylist);
        logger.debug('\n' + 'loading playlist {{playlist_name}}: finished');

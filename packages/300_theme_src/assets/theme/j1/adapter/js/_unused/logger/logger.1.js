...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/logger.js (1)
...
j1.adapter.logger = ((j1, window) => {
  ...
  return {
    ...
    init: (options) => {
      ...
      // stays 'na' (e.g. cookies disabled, an early init error elsewhere),
      // this 10ms interval would run for the lifetime of the tab WITHOUT
      // any logger appenders attached — so the failure would also be
      // silent. Cap at 30s and report via console.warn directly because
      // the log4javascript logger isn't usefully wired up yet at this
      // point in the boot sequence.
      //
      dependenciesTimeout = setTimeout(function () {
        if (dependencies_met_mode_detected) {
          clearInterval(dependencies_met_mode_detected);
          // jadams: console.warn intentional — see comment above.
          // eslint-disable-next-line no-console
          console.warn('logger init aborted: user_session.mode not set within 5s');
        }
      }, 5000);
    }, // END init

    ...
    init: (options) => {
      ...
      var dependencies_met_page_ready = setInterval (() => {
        ...
        if (j1CoreFinished && pageVisible && atticFinished) {
          ...
          // -------------------------------------------------------------------
          // load all amplitude playlists
          // -------------------------------------------------------------------
          _this.playlistLoader(playLists);
          // -------------------------------------------------------------------
          // load all amplitude players (HTML)
          // -------------------------------------------------------------------
          _this.playerLoader(playerUIsLoaded);
...
//          logger.info('\n' + 'loading playlist {{player.playlist}}: started');
            ...
            // -----------------------------------------------------------------
            // initialize amplitude instance (when player UI is loaded)
            // -----------------------------------------------------------------
...
//              _this.initApi(playList, playListTitle, playerItemsEnabled);
                _this.initApi(playerItemsEnabled, playLists);
    ...
    playlistLoader: (playlist) => {
      ...
      {% for player in amplitude_options.players %} {% if player.enabled %}
        ...
        for (var i = 0; i < Object.keys(playerItems).length; i++) {
          ...
          // prevent multiple processing of already defined playlist
          ...
          if (playerItems[i].enabled) {
            ...
            // logger.info('\n' + 'loading playlist {{playlist_name}}: started');
        ...
        logger.info('\n' + 'loading playlists: finished');
    }, // END playlistLoader

    // -------------------------------------------------------------------------
    // Player loader
    // -------------------------------------------------------------------------
    playerLoader: (playersLoaded) => {

      // -----------------------------------------------------------------------
      // initialize HTML portion (UI) for all players configured|enabled
      // -----------------------------------------------------------------------
      logger.info('\n' + 'loading players: started');
      ...
      {% for player in amplitude_options.players %} {% if player.enabled %}
        ...
        logger.info('\n' + 'loading player on ID #{{player_id}}: started');
        ...
        load_dependencies['dependencies_met_html_loaded_{{player_id}}'] = setInterval (() => {
          ...
          if (xhrLoadState === 'success') {
            logger.info('\n' + 'loading player on ID #{{player_id}}: finished');

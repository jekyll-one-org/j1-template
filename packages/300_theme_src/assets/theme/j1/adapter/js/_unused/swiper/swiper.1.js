...
j1.adapter.swiper = ((j1, window) => {
  ...
  // function replaceNestedValues(obj, regex, replacement) {
  //     for (const key in obj) {
  //       if (typeof obj[key] === 'object') {
  //         replaceNestedValues(obj[key], regex, replacement); 
    ...
    init: (options) => {
      ...
      var dependencies_met_page_ready = setInterval (() => {
        ...
        if (j1CoreFinished && pageVisible && atticFinished) {
          ...
          // initialize swiper HTML portion
          load_dependencies['dependencies_met_html_loaded_{{swiper.id}}'] = setInterval (() => {
            // check swiper HTML portion is loaded successfully
            ...
            if (xhrLoadState === 'success') {
              ...
              var swiperOptions         = {{swiper.options | replace: '=>', ':' }};
              var swiperOptionsJSON     = JSON.stringify(swiperOptions, null, 2);
              var swiperOptionsString   = swiperOptionsJSON.replace(/{|}/g, '');

              const {{swiper.id}} = new Swiper('#{{swiper.id}}', swiperOptionsString);

              // var regexPattern = "(?m)^\h*\K"|"(?=\h*$)";
              // var regObj = new RegExp(patt1, "gm");
              // var obj = obj.replace(regObj, "");

              // const newObject = replaceInNestedObjects(swiperOptions, /^"/gm, '');

              // Object.keys(swiperOptions).forEach(k => str = str.replace(new RegExp(`(?m)^\h*\K"|"(?=\h*$)`, 'g'), state[k]));

              // setup slider configuratiion (parameters|options)
              logger.info ('\n' + 'swiper is being setup on id: ' + '{{swiper.id}}');

              // {% for setting in swiper.modules %}
              // {% capture modules %} {{setting[0]}}: {{setting[1] | replace: '=>', ':'}}, {% endcapture %}
              // {% endfor %}  

              // var modules         = { {{modules}} };
              // var modulesJSON     = JSON.stringify(modules, null, 2);
              // var modulesString   = modulesJSON.replace(/{|}/g, '');

//            const {{swiper.id}} = new Swiper('#{{swiper.id}}', mySwiperOptions);
//            const {{swiper.id}} = new Swiper('#{{swiper.id}}', swiperOptions);

              // const {{swiper.id}} = new Swiper('#{{swiper.id}}', {
              //   // parameters
              //   {% for setting in swiper.options %}
              //   {{setting[0]}}: {{setting[1]}},
              //   {% endfor %}
              //   // modules
              //   // ...modules,
              ...
              //   // {% for setting in swiper.modules %}
              //   // {{setting[0]}}: {{ setting[1] | replace: '=>', ':' | jsonStr }},
              //   // {% endfor %}

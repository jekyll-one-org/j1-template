...
//              var scrollDuration          = 300;
    ...
    registerMonitors: () => {
      ...
      //         let result = confirm("Seitenwechsel erkannt. Sind Sie sicher?");
      //         if (result) {
      //             console.warn("Neue Seite: " + document.location);
      //             console.log("Der Benutzer hat OK geklickt.");
      //             previousUrl = currentUrl;
      //         } else {
      //             console.warn("Neue Seite: " + document.location);
      //             console.log("Der Benutzer hat Abbrechen geklickt.");
      //         }              
      //       }
      //     }
      //   }
...
//          if (growthRatio >= 5) {
    ...
    reloadMonitor: () => {
      ...
      var dependencies_met_page_ready = setInterval (() => {
        ...
        if (j1CoreFinished && pageVisible) {
          script.id = 'reloadMonitor';

          // Inline script
          script.innerHTML = `
            window.onpopstate = function(event) {
              // Hier wird der Code ausgeführt, wenn sich der Zustand des History-Stacks ändert
            ...
            window.addEventListener('beforeunload', function(event) {
              var message = 'Balla Kollega? Sicher, dass Sie diese Seite verlassen möchten?';
              // event.returnValue = message; // Für ältere Browser
              return message; // Für modernere Browser
            });        
          `;

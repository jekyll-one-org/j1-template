...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/masonry.js (3)
            ...
            {% if grid.enabled %}
              ...
              load_dependencies['dependencies_met_html_loaded_{{grid.id}}'] = setInterval (() => {
                ...
                if (xhrLoadState === 'success') {
                  ...
                  // claude - Fix Masonry module error #2
                  // Defer the grid setup until the container reports a
                  // non-zero width. The root cause of the (former) hard
                  // failure "RangeError: Invalid array length" was the
                  // adapter initializing Masonry before the container is
                  // laid out (attached, visible and measurable). Fix #1 in
                  // the core module already turned that hard failure into a
                  // harmless single-column first paint. This fix removes the
                  // trigger itself on the adapter side: the original setup
                  // body (unchanged below) is wrapped into the named
                  // function initGrid_{{grid_id}}(). Before the body runs,
                  // the container width is probed; while it is still zero,
                  // the setup is re-scheduled with a bounded retry
                  // (maxInitAttempts x initRetryDelay). If the container
                  // never becomes measurable within the retry budget, the
                  // setup runs anyway as a last resort — the core module's
                  // Fix #1 guards keep that path safe (single-column
                  // fallback, recomputed on the next resize/layout pass).
                  //
                  var initAttempts_{{grid_id}}    = 0;
                  var maxInitAttempts_{{grid_id}} = 50;
                  var initRetryDelay_{{grid_id}}  = 100;

                  var initGrid_{{grid_id}} = () => {

                    // claude - Fix Masonry module error #2
                    // width gate: re-schedule the setup while the container
                    // is not (yet) measurable and the retry budget lasts.
                    // The container is re-queried on every attempt because
                    // the XHR-loaded DOM may be replaced between attempts.
                    //
                    var gridContainer_{{grid_id}}  = document.getElementById('{{grid_id}}');
                    var gridMeasurable_{{grid_id}} = gridContainer_{{grid_id}} && gridContainer_{{grid_id}}.offsetWidth > 0;
                    if (!gridMeasurable_{{grid_id}} && initAttempts_{{grid_id}} < maxInitAttempts_{{grid_id}}) {
                      initAttempts_{{grid_id}}++;
                      logger.debug('\n' + 'container not (yet) measurable on id: {{grid_id}}, defer grid setup (attempt: ' + initAttempts_{{grid_id}} + ')');
                      setTimeout(initGrid_{{grid_id}}, initRetryDelay_{{grid_id}});
                      return;
                    }
                  ...
                  }; // END initGrid_{{grid_id}}

                  // claude - Fix Masonry module error #2
                  // initial (deferred) setup call. The configured delay
                  // masonryOptions.initTimeout (masonry.yml) is kept
                  // unchanged as the first-attempt delay; the width gate
                  // inside initGrid_{{grid_id}}() takes over from there.
                  //
                  setTimeout(initGrid_{{grid_id}}, masonryOptions.initTimeout);
                }

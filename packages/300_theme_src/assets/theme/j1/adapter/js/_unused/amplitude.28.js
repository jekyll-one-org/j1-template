...
j1.adapter.amplitude = ((j1, window) => {
  ...
  return {
      ...
      var dependencies_met_player_instances_initialized = setInterval (() => {
        if (apiInitialized.state) {
          ...
          {% for player in amplitude_options.players %} {% if player.enabled %}
            ...
            pluginManagerEnabled  = ('{{player.plugin_manager.enabled}}' === 'true') ? true : playerDefaultPluginManager;
    ...
    // -------------------------------------------------------------------------
    pluginManager: () => {
      // var dependencies_met_ajs_player_ready = setInterval (() => {
      //   var ajsPlayerAvailable = ($('#mimi_rutherfurt_yt_large').length > 0) ? true : false;

        // if (ajsPlayerAvailable) {
          // load the AJS Player plugin (tech)
          var techScript;
      ...
      //     clearInterval(dependencies_met_ajs_player_ready);
      //   } // END pageVisible
      // }, 10); // END dependencies_met_ajs_player_ready
    }, // END pluginManager

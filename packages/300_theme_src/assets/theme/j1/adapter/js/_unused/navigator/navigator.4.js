...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/navigator.js (3)
    ...
    init: (options) => {
      ...
      var dependencies_met_module_initialized = setInterval (() => {
        ...
        {% endif %}
          ...
          // Fix J1 Navigator issue #2
          // -------------------------------------------------------------------
          // Register event 'click' on all dropdown items (li.dropdown-item)
          // of the NAV main menu to mark the clicked item as "active".
          // Before the class "active" is added on the clicked item, the
          // class is removed from all other dropdown items of the menu.
          //
          // NOTE: The handler is registered as a DELEGATED event on the
          // NAV bar element (nav.navbar.navigator). This way, the handler
          // works for all dropdown items, even if (parts of) the menu are
          // created or recreated dynamically (e.g. by manageDropdownMenu
          // on window resize events for desktop|mobile view).
          // -------------------------------------------------------------------
          //
          $('nav.navbar.navigator').on('click', 'li.dropdown-item', function () {
            var clickedItem = $(this);

            // remove the class "active" from ALL dropdown items of the menu
            $('nav.navbar.navigator li.dropdown-item').removeClass('active');

            // add the class "active" on the CLICKED dropdown item (only)

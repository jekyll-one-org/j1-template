...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/videoPlayer.js (28)
 ...
 # videoPlayer_control.yml) site 1 crashed on the first playlist toggle:
 #
 ...
 #   3. _data/modules/videoPlayer_control.yml   (players[]  ... )   per player
 #
 ...
 # per-player keys of videoPlayer_control.yml (videoJS.autoStart,
 ...
 #
 # Changes (all tagged "claude - Modify J1 VideoPlayer #48"):
 #
 ...
 #       <- per-player entry (videoPlayer_control.yml, players[].ui_elements)
 ...
 #
 # Changes (all tagged "claude - Modify J1 VideoPlayer #52"):
 #
...
-------------------------------------------------------------------------------- {% endcomment %}
{% assign videoplayer_options     = videoplayer_default | merge: videoplayer_settings %}
...
j1.adapter.videoPlayer = ((j1, window) => {
  ...
  var videoPlayerInstanceOptions = {};
  ...
  return {
    ...
    init: (options) => {
      ...
      //
      videoPlayerInstanceOptions = {};
      _this['videoPlayerInstanceOptions'] = videoPlayerInstanceOptions;
      ...
      var dependencies_met_page_ready = setInterval(() => {
        ...
        if (j1CoreFinished && pageVisible && j1AtticFinished) {
          ...
          {% for video_player in players %}
            ...
            -------------------------------------------------------------------- {% endcomment %}
            {% assign player = videoplayer_default | merge: videoplayer_settings | merge: video_player %}
            ...
            {% if player.enabled %}
              ...
              load_dependencies['dependencies_met_html_loaded_{{player_id}}'] = setInterval (() => {
                ...
                if (xhrLoadState === 'success' && typeof videoPlayer !== 'undefined') {
                  ...
                  // instead of the global videoPlayerOptions, so the module
    ...
    //
    //   1. videoPlayerDefaults  — _data/modules/defaults/videoPlayer.yml
    //   2. videoPlayerSettings  — _data/modules/videoPlayer.yml
    //   3. player entry         — _data/modules/videoPlayer_control.yml
    //                             (videoPlayers.players, matched by id)
    //
    ...
    // j1.adapter.videoPlayer.videoPlayerInstanceOptions[playerId] so the
    // module can read the per-instance options directly.
    // -------------------------------------------------------------------------
    getInstanceOptions: (playerId) => {
      // fast path: already resolved for this instance
      if (videoPlayerInstanceOptions[playerId]) {
        return videoPlayerInstanceOptions[playerId];
      }
      ...
      // update environment setting (parity with the global videoPlayerOptions)
      instanceOptions.env = env;

      // cache + expose
      videoPlayerInstanceOptions[playerId] = instanceOptions;
      _this['videoPlayerInstanceOptions']  = videoPlayerInstanceOptions;

...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/fab.js (1)
...
j1.adapter.fab = ((j1, window) => {
  ...
  return {
    ...
    init: (options) => {
      ...
      pageReadyTimeout = setTimeout(function () {
        if (dependency_met_page_ready) {
          clearInterval(dependency_met_page_ready);
          logger.warn('fab init aborted: page-ready conditions not met within 30s');
        }
      }, 30000);
    }, // END init
    ...
    fabLoader: (fabOptions) => {
      ...
      // missing/wrong xhr_container_id in fab.yml all leave this state
      // permanently as `undefined` or `'pending'`, in which case the
      // 10ms interval runs for the lifetime of the tab. Cap at 30s and
      // log a warning that names the container id so the failing fetch
      // is debuggable from the console.
      //
      fabInitTimeout = setTimeout(function () {
        if (dependencies_met_fab_initialized) {
          clearInterval(dependencies_met_fab_initialized);
          logger.warn('fab init aborted: HTML for #' + fabOptions.xhr_container_id + ' did not load within 30s (xhrDOMState=' + j1.xhrDOMState['#' + fabOptions.xhr_container_id] + ')');
        }
      }, 30000);
    }, // END fabLoader
              ...
              if (eventHandler.split(' ').length == 1) {
                ...
                if (eventHandler === 'open_mmenu_toc') {
                  if ($('#j1-toc-mgr').length) {
                    ...
                    var dependencies_met_toccer_finished = setInterval (() => {
                      if (j1.adapter.toccer.getState() == 'finished') {
                        ...
                        if (toccerTimeoutMulti) {
                          clearTimeout(toccerTimeoutMulti);
                          toccerTimeoutMulti = null;
                        }
                      }
                    ...
                    // interval would run for the lifetime of the tab. Cap
                    // at 30s and log a warning. Note: declared with `var`
                    // and a unique name so multiple FAB items configured
                    // with `open_mmenu_toc` don't shadow each other across
                    // forEach iterations.
                    //
                    var toccerTimeoutMulti = setTimeout(function () {
                      if (dependencies_met_toccer_finished) {
                        clearInterval(dependencies_met_toccer_finished);
                        logger.warn('fab open_mmenu_toc: toccer adapter did not finish within 5s');
                      }
                    }, 5000);
                  } else {
            ...
            if (eventHandler.split(' ').length === 1) {
              ...
              if ( eventHandler === 'open_mmenu_toc' ) {
                ...
                if ($('#j1-toc-mgr').length) {
                  ...
                  var dependencies_met_toccer_finished = setInterval (() => {
                    if (j1.adapter.toccer.getState() === 'finished') {
                      ...
                      if (toccerTimeoutSingle) {
                        clearTimeout(toccerTimeoutSingle);
                        toccerTimeoutSingle = null;
                      }
                    }
                  }, 10); // END dependencies_met_toccer_finished
                  ...
                  // warning. Unique variable name keeps it independent of
                  // the multi-action branch's timeout above.
                  //
                  var toccerTimeoutSingle = setTimeout(function () {
                    if (dependencies_met_toccer_finished) {
                      clearInterval(dependencies_met_toccer_finished);
                      logger.warn('fab single-action open_mmenu_toc: toccer adapter did not finish within 30s');
                    }
                  }, 30000);
                } else {
                  logger.info('found toc in page: disabled');

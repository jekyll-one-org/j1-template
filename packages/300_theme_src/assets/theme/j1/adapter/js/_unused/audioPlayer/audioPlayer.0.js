...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/audioPlayer.js (0)
...
j1.adapter.audioPlayer = ((j1, window) => {
  ...
  return {
    ...
    init: (options) => {
      ...
      // claude - Fix Amplitude YAML data processing #1
      ...
      // j1.adapter.amplitude.getInstanceOptions(playerId).
      //
      ...
      // set INITIAL AudioPlayerJS data
      //
      j1.modules.audioplayerjs.data.atp.activeIndex           = false;
      j1.modules.audioplayerjs.data.atp.playlist              = false
      j1.modules.audioplayerjs.data.atp.apiError              = false;
      j1.modules.audioplayerjs.data.atp.apiReady              = false ;
      j1.modules.audioplayerjs.data.atp.playlist              = false;
      j1.modules.audioplayerjs.data.atp.players               = {};
      j1.modules.audioplayerjs.data.ytp.apiError              = false;
      j1.modules.audioplayerjs.data.ytp.apiReady              = false;
      j1.modules.audioplayerjs.data.ytp.players               = {};
      j1.modules.audioplayerjs.data.ytp.plugin                = false;

      // save audioplayerjs data for later use (e.g. events)
      //
      ...
      var dependencies_met_page_ready = setInterval (() => {
        ...
        var ampltitudeFinished  = (j1.adapter.amplitude.getState() == 'finished') ? true : false;
        
        if (j1CoreFinished && pageVisible && atticFinished && ampltitudeFinished) {
      ...
      {% for playlist in audioplayer_media.playlists %} {% if playlist.enabled %}
        ...
        for (var i = 0; i < Object.keys(song_items).length; i++) {
          if (song_items[i].enabled) {
            ...
            // map config settings|audioplayer song items
            // -----------------------------------------------------------------
    ...
    playerHtmlLoader: (playersLoaded) => {
      ...
      {% for player in audioplayer_control.players %} {% if player.enabled %}
        ...
        if (playerExistsInPage) {
          ...
          // -------------------------------------------------------------------
          // initialize audioplayer instance (when player UI loaded)
          // -------------------------------------------------------------------
    ...
    initApi: (songlist) => {
      ...
      function atpFadeAudioOut(params) {
        ...
        // ---------------------------------------------------------------------
        j1.modules.audioplayerjs.data.atp.activeIndex = songIndex;
        j1.modules.audioplayerjs.data.atp.playlist = playlist;
      ...
      function onInitialized() {
        ...
        j1.modules.audioplayerjs.data.atp.apiReady = apiInitialized.state;
      } // END onInitialized
                  ...
                  if (apiInitialized.state) {
                    ...
                    if (document.getElementById('{{player.id}}') !== null) {
                      ...
                      if (volumeSlider !== null) {
                        ...
                        // if player has NO slider presets, use audioplayer defaults
                        //
    ...
    getInstanceOptions: (playerId) => {
      ...
      var instanceOptions = $.extend({}, audioPlayerDefaults, userSettings);
      ...
      if (instanceOptions.player === undefined || instanceOptions.player === null) {
        ...
        instanceOptions.player = $.extend({}, instanceOptions.player, playerEntry);
      }
      ...
      j1.adapter.audioPlayer.audioPlayerInstanceOptions = audioPlayerInstanceOptions;

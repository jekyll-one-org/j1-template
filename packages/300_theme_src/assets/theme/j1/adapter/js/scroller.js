...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/scroller.js (2)
...
//      var j1AtticFinished = (j1.adapter.attic.getState() === 'finished') ? true : false;
        ...
        if (j1CoreFinished && pageVisible) {
          ...
          // jadams, workaround: page loaded dynamically
          setTimeout(() => {
    ...
    generate_scrollers: () => {
      ...
      // J1 Adapter optimizations #2
      // page-aware filtering. The YAML configuration defines scrollers
      // globally for the whole site, but the adapter is loaded per page.
      // Without this guard, every page would attempt to initialize every
      // configured scroller and poll for containers that exist only on
      // other pages (e.g. #timeline on a non-blog-navigator page),
      // emitting a misleading "container poll aborted" warning after 5s.
      // We do a synchronous existence check up front and skip scrollers
      // whose wrapper is not present in the current page's DOM.
      //
      if (document.getElementById('{{item.scroller.container}}') === null) {
        logger.debug('scroller {{item.scroller.id}} skipped: wrapper #{{item.scroller.container}} not present on this page');
      } else {
        logger.debug('scroller {{item.scroller.id}} is being initialized on wrapper: {{item.scroller.container}}');
        ...
        wrapper_dependencies['dependency_met_wrapper_ready_{{item.scroller.container}}'] = setInterval(() => {
          ...
          // J1 Adapter optimizations #2
          // fix always-true predicate. getElementById returns either an
          // Element or null (never undefined), and `null !== undefined` is
          // true, so the original OR-chain was always truthy and the inner
          // branch ran unconditionally. Use a strict null check.
          //
      ...
      } // END else: wrapper present on this page (J1 Adapter optimizations #2)

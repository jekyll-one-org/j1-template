...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/rouge.js (1)
    ...
    init: (options) => {
      ...
      dependenciesTimeout = setTimeout(() => {
        if (dependency_met_page_ready) {
          clearInterval(dependency_met_page_ready);
          logger.warn('rouge init aborted: page-ready conditions not met within 5s');
        }
      }, 5000);
      ...
      // the tab. Cap it at 30s with the same diagnostic warning.
      //
      var rougeFinishedTimeout;
      var dependencies_met_rouge_finished = setInterval(() => {
        ...
        if (moduleFinished) {
          ...
          if (rougeFinishedTimeout) {
            clearTimeout(rougeFinishedTimeout);
            rougeFinishedTimeout = null;
          }
        } //  END if darkTheme
      ...
      rougeFinishedTimeout = setTimeout(() => {
        if (dependencies_met_rouge_finished) {
          clearInterval(dependencies_met_rouge_finished);
          logger.warn('rouge style apply aborted: module finished state not reached within 5s');
        }
      }, 5000);
    }, // END init

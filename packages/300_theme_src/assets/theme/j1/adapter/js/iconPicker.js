...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/adapter/js/iconPicker.js (1)
    ...
    init: (options) => {
      ...
      var dependencies_met_page_ready = setInterval((options) => {
        ...
        if (j1CoreFinished && pageVisible) {
          ...
          var dependencies_met_picker_button_ready = setInterval (() => {
            ...
            if (buttonReady) {
              ...
              if (pickerButtonTimeout) {
                clearTimeout(pickerButtonTimeout);
                pickerButtonTimeout = null;
              }
            } // END if buttonReady
          ...
          pickerButtonTimeout = setTimeout(function () {
            if (dependencies_met_picker_button_ready) {
              clearInterval(dependencies_met_picker_button_ready);
              logger.warn('iconPicker init aborted: button ' + icon_picker_button_id + ' not found within 5s');
            }
          }, 5000);
      ...
      pageReadyTimeout = setTimeout(function () {
        if (dependencies_met_page_ready) {
          clearInterval(dependencies_met_page_ready);
          logger.warn('iconPicker init aborted: page-ready conditions not met within 30s');
        }
      }, 30000);
    }, // END init

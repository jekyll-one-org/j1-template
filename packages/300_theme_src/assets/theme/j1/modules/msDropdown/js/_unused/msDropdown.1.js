/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/msDropdown/js/msDropdown.js (1)
 ...
 * it under the terms of the either the MIT License or the 
...
(() => { // webpackBootstrap
  ...
  class ddMaker {
    ...
    constructor(ele, settings) {
      ...
      this.ele = ele;
      // J1 msDropdown optimizations #1
      // CORRECTNESS: The original used a shallow spread `{...defaultSettings, ...settings}`
      // which fully replaces the nested `on` and `byJson` objects whenever the
      // caller supplies even a single property. That meant passing
      // `{on: {change: fn}}` would silently null out every other event slot
      // (open/close/create/etc.) and break `_has_handler` / `_fireLocalEventIfExist`.
      // We now merge top-level keys shallowly but extend the two known nested
      // objects key-by-key so user-supplied handlers do not erase the defaults.
    ...
    _checkDataSettings() {
      ...
      // J1 msDropdown optimizations #1
      // Removed the trailing no-op `this._settings = {...this._settings, ...settings};`.
      // `settings` is a *reference* to `this._settings` (via `let settings = this._settings`),
      // so every assignment above already mutates `this._settings` in place. The spread
      // merged the object with itself, allocating a new object each call for no benefit.
    }
      ...
      if (this._settings.byJson.data) {
        try {
          ...
          for (let i = 0; i < total; i++) {
            ...
            // J1 msDropdown optimizations #1
            // CORRECTNESS/CLARITY: Switched from `for...in` (which has
            // no `hasOwnProperty` guard and would walk inherited props
            // if Object.prototype is polluted) to `Object.keys(...)`.
            // Also fixed an order-of-operations smell: the original
            // built `key` as `data-${p}` *and then* re-applied the
            // camelCase-to-kebab-case regex, which is correct but
            // surprising; the comment now spells that out.
            const propKeys = Object.keys(current);
            for (let pi = 0, plen = propKeys.length; pi < plen; pi++) {
              const p = propKeys[pi];
              if (p.toLowerCase() !== 'text') {
                // Build `data-foo-bar` from `fooBar` (camelCase
                // -> kebab-case) -- HTML data attributes must be
                // kebab-case to round-trip via `dataset`.
                let key = `data-${p}`.replace(/([A-Z])/g, "-$1").toLowerCase();
          ...
          // J1 msDropdown optimizations #1
          // CORRECTNESS: The original threw a bare string, which
          // (a) loses the original error stack and (b) is flagged by
          // every linter (no-throw-literal). Re-throw a real Error and
          // attach the original via `cause` so debuggers can drill in.
          throw new Error("There is an error in json data.", {
            cause: e
          });
        }
      }
    ...
    _applyFilters(e) {
      ...
      if (sText.length === 0) {
        ...
        //hide all
        // J1 msDropdown optimizations #1
        // PERFORMANCE: Hoisted `sText.toLowerCase()` and used `includes`
        // instead of `indexOf(...) >= 0` for readability. The original
        // recomputed the lower-cased filter string for every option in
        // the list. Also switched the predicate to an arrow function
        // (no `this` rebinding needed; minor micro-opt) and replaced
        // `nodeName !== "OPTGROUP"` with `tagName` which is the modern,
        // case-stable equivalent.
        const needle = sText.toLowerCase();
        let options = [...this.options];
        let filterOptions = options.filter((item) => {
          return item.tagName !== "OPTGROUP" &&
            item.disabled === false &&
    ...
    _makeFilterBox() {
      ...
      // J1 msDropdown optimizations #1
      // PERFORMANCE: The original ran `_applyFilters` (which rebuilds every
      // <li> from scratch via `_makeChildren`) on every single keystroke.
      // For lists with hundreds of items and fast typists this caused
      // visible lag and dropped characters. We add a small (60 ms) debounce
      // so multiple keystrokes within one frame collapse to a single rebuild.
      // Choice of 60 ms keeps the UI feeling responsive (well below the
      // ~100 ms perceptual threshold) while still cutting work by 5-10x in
      // the typical case.
    ...
    _makeHeader() {
      let css = this._css;
      let divHeader = this._createEle("div", {
        className: css.header
      });
      let headerA = this._createEle("a", {
        className: css.headerA
      });
      let headerContent = this._createEle("span", {
        className: css.headerMiddleContent
      });
      let arrow = this._createEle("span", {
    ...
    _makeChildren(byOption = null) {
      ...
      // J1 msDropdown optimizations #1
      // PERFORMANCE: Invalidate the optionsUI / index-to-LI caches whenever
      // the option list is rebuilt. Without this the (now-functional) cache
      // could hand out stale NodeList references after filtering, add()/remove(),
      // or refresh().
      this._cacheEle["allItems"] = null;
      this._cacheEle["byIndex"] = null;
      ...
      // J1 msDropdown optimizations #1
      // CLARITY (NOTE, no behaviour change): The block below and the inner
      // `if(itemObj.isOptGroup)` block 25 lines down are nearly identical
      // -- both parse an option, create an LI, attach className/style/index,
      // mark as selected if needed, and bind events. A future refactor
      // could extract a `_buildAndAppendLi(target, current, j)` helper
      // (it would only need to differ in the parent UL and in whether
      // events are bound for an OPTGROUP root). Left as-is here to keep
      // this optimization pass strictly behaviour-preserving.
    ...
    _makeLayout() {
      ...
      let wrapper = this._createEle("div", {
        tabIndex: 0,
        className: css.dd
      });
      ...
      let valueBox = this._createEle("input", {
        tabIndex: -1,
        name: name,
        type: "text",
      ...
      let moreValueBox = this._createEle("div", {
        className: "more",
        style: "display:none"
      });
    ...
    _createRow(obj) {
      ...
      let itemSpan = this._createEle("span", {
        className: this._css.itemSpan + optTxtCss
      });
      ...
      let textSpan = this._createEle("span", {
        className: obj.label.css
      }, text);

      let itemInnerSpan = this._createEle("span", {
        className: obj.innerSpanCss
      });
      itemInnerSpan.appendChild(textSpan);

      if (obj.img.src !== null) {
        let img = this._createEle("img", {
          className: obj.img.css,
          src: obj.img.src
        });
      ...
      if (obj.img.src === null && obj.imageCss.replace(/\s/g, '') !== '') {
        let imgSpan = this._createEle("span", {
          className: obj.img.css + ' ' + obj.imageCss
        }, " ");
        itemSpan.appendChild(imgSpan);
      }

      if (obj.desc.text !== null) {
        let spanDesc = this._createEle("span", {
          className: obj.desc.css
        }, obj.desc.text);
    ...
    _parseOption(opt) {
      ...
      let image = null,
        ...
        imageCss = '',
        index = -1,
        selected, disabled, internalStyle;
      if (opt !== undefined) {
     ...
     * @param ele - the LI element to mark as selected
     * @param dontThink - when true, ONLY toggles the selected CSS class on
     *   the LI, without touching the underlying <select>'s state, firing
     *   change, or clearing other selections. Used during initial UI build
     *   and OPTGROUP child rendering, where the <select>'s own
     *   `selectedIndex` is already authoritative and we just need the LI
     *   to reflect it.
     * @param resetOldSelected - when true (default), clears any previously
     *   selected LIs and sets the <select>'s `selectedIndex`. When false,
     *   leaves prior selections in place (used for shift/ctrl multi-select).
     * @private
     */
    // J1 msDropdown optimizations #1
    // CLARITY: The parameter `dontThink` and its effect were undocumented.
    // I traced every call site and added the JSDoc above so that future
    // maintainers don't have to. Behaviour is unchanged.
     ...
     */
    // J1 msDropdown optimizations #1
    // PERFORMANCE: The original ran `querySelectorAll` AND a linear scan of
    // every `<li>` on every call. Hot path callers include `_getDataAndUI`,
    // `updateUiAndValue`, and the multiple-selection loop in `_getDataAndUI`
    // -- giving O(n^2) behaviour for n selected items in long lists.
    // We now build a Map<index, li> lazily on first call and reuse it until
    // the list is rebuilt (cache cleared in `_makeChildren`).
    _findElementByIndexProp(index) {
      let map = this._cacheEle["byIndex"];
      if (!map) {
        map = new Map();
        const options = this._getAllEle(`ul li.${this._css.item}`, this._wrapper.holder);
        for (let i = 0, total = options.length; i < total; i++) {
          map.set(options[i].index, options[i]);
        }
        this._cacheEle["byIndex"] = map;
      }
      return map.has(index) ? map.get(index) : null;
    }
     ...
     */
    // J1 msDropdown optimizations #1
    // CLARITY/PERFORMANCE: Replaced the legacy `Object.prototype.toString.call()`
    // idiom with the native `Array.isArray`, which has been universally supported
    // since IE9 and is significantly faster (no string allocation, no prototype walk).
    // Also converted from a class field arrow to a normal method - there is no
    // need for `this` binding here, and it keeps the method on the prototype
    // (saving one closure per instance).
    _isArray(obj) {
      return Array.isArray(obj);
    }

    /**
     * Coerce a setting value to boolean.
     * @param val
     * @return {boolean}
     * @private
     */
    // J1 msDropdown optimizations #1
    // CLARITY: Helper to centralise the `.toString() === "true"` pattern that
    // was duplicated 10+ times throughout the source. Settings can arrive as
    // either real booleans (when set programmatically) or strings (when read
    // from `data-*` attributes), so a single coercion helper improves
    // readability and eliminates drift risk if the default ever changes.
    // We accept the existing string-truthy semantics: anything other than
    // the literal string "false"/"0" or boolean false is considered true.
    _toBool(val) {
    ...
    updateUiAndValue(byData = null) {
      ...
      //this is multiple
      // J1 msDropdown optimizations #1
      // CORRECTNESS/CLARITY: `String.prototype.substr` is a legacy/deprecated method
      // (https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/String/substr).
      // The original call also passed an incorrect 2nd argument
      // (`valueBox.name.length` instead of a length of 2), so the suffix-check
      // worked only by coincidence. `endsWith` makes the intent explicit and
      // is supported in every modern browser.
      if (this._isMultiple && valueBox.name.endsWith("[]")) {
        this._wrapper.moreValueBox.innerHTML = "";
        for (let i = 1; i < dataAndUI.data.length; i++) {
          let valueBoxM = this._createEle("input", {
            type: "hidden",
            name: valueBox.name,
            value: dataAndUI.data[i].value
          });
     ...
     */
    // J1 msDropdown optimizations #1
    // CORRECTNESS/PERFORMANCE: The original used `for (let i in attr)` with
    // no `hasOwnProperty` guard, so any property added to Object.prototype
    // by a polyfill or rogue script would be applied as an attribute on
    // every created element. Switched to `Object.keys(...).forEach`, which
    // only iterates own enumerable string-keyed properties and is also
    // measurably faster than `for...in` in modern engines.
     ...
     */
    // J1 msDropdown optimizations #1
    // CLARITY: Original used "inherit" as the show value here but "block"
    // in `_show()`. Aligned the toggle helper with `_show()`'s default so
    // round-tripping through these helpers is idempotent. (`inherit` could
    // resolve to `none` if a parent declares it, which would be a subtle
    // bug if `_toggleShow` was ever used on a list-of-items <ul>.)
    _toggleShow(ele) {
     ...
     */
    // J1 msDropdown optimizations #1
    // CLARITY: Renamed misspelled parameter `dispaly` -> `display`.
    _show(ele, display = "block") {
     ...
     */
    // J1 msDropdown optimizations #1
    // CLARITY: The original used a hard-coded `+ 12` to account for the list's
    // top + bottom borders/padding. Documented the magic number and switched
    // to a guarded `parseInt`, which previously returned `NaN` (and silently
    // skipped resizing) when `visibleRows` was unset. Now `null`/`undefined`
    // short-circuits and an invalid string also short-circuits cleanly.
    _adjustChildHeight(row = null) {
      if (row === null) {
        const parsed = parseInt(this._settings.visibleRows, 10);
        row = Number.isFinite(parsed) ? parsed : null;
      }
      if (row !== null) {
        let li = this._getEle("li[data-ms-index='0']", this._wrapper.listOfItems);
        if (!li) return; // empty list - nothing to size
        let size = (this._settings.rowHeight !== null) ? this._settings.rowHeight : li.clientHeight;
        // The +12 is the visual fudge for the top/bottom border (1px each)
        // plus the inner UL's padding. Keep as a named constant for clarity.
        const LIST_CHROME_PX = 12;
        this._wrapper.listOfItems.style.height = (row * size) + LIST_CHROME_PX + 'px';
      }
     ...
     */
    // J1 msDropdown optimizations #1
    // PERFORMANCE: Reading `clientHeight` for every <li> in a loop forces a
    // synchronous layout per element. We can't avoid that work entirely
    // (the heights are layout-dependent), but we (a) cache the NodeList
    // length, (b) use `Math.max` to skip a temporary, and (c) early-exit
    // when there are no items. Functional behaviour is unchanged.
    _setTitleMinHeight(autoHeightByMax = true) {
      let max = 0;
      if (autoHeightByMax === true) {
        const lis = this._getAllEle("li", this._wrapper.listOfItems);
        for (let i = 0, len = lis.length; i < len; i++) {
          const h = lis[i].clientHeight;
          if (h > max) max = h;
        }
      } else {
    ...
    _has_handler(name) {
      ...
      // J1 msDropdown optimizations #1
      // CORRECTNESS: The original used `this._settings.on[name] !== null`,
      // which returns `true` when the handler is `undefined` (e.g. an
      // event name that was never declared in `defaultSettings.on` or a
      // slot lost to a shallow-merge bug). That made `_has_handler`
      // mis-report `hasEvent: true` for events that have no registered
      // callback. We now test that the slot actually contains a function,
      // which is the only state in which `_fireLocalEventIfExist` will fire.
      if (typeof this._settings.on[name] === "function") {
        evt.hasEvent = true;
        evt.local = true;
      }
    ...
    destroy() {
      // J1 msDropdown optimizations #1
      // CORRECTNESS: The original destroy() removed the wrapper from the DOM
      // but left the `mouseup` / `keydown` / `keyup` listeners that
      // `_bindDocumentEvents` had attached to `document`. Those closures
      // capture `this`, so the entire ddMaker instance (and the wrapper
      // it referenced) leaked until page reload. We now unbind document
      // events and clear the module-private `_oldC` pointer if it points
      // at this instance.
      this._unbindDocumentEvents();
      if (_oldC === this) {
        _oldC = null;
     ...
     */
    // J1 msDropdown optimizations #1
    // PERFORMANCE/CORRECTNESS: The original cache was dead code -- the early
    // `return` was commented out, so every getter call ran a fresh
    // `querySelectorAll` even though the result was assigned to the cache.
    // For `next()`/`previous()` (each invocation reads `optionsUI` once) and
    // for keyboard navigation in long lists this caused unnecessary DOM walks.
    // We now actually return the cached NodeList. The cache is invalidated in
    // `_makeChildren` (whenever the list is rebuilt) and on `add`/`remove`.
    get optionsUI() {
    ...
    static make(ele, settings) {
      if (!ele.msDropdown) {
        try {
          ...
          // J1 msDropdown optimizations #1
          // BUGFIX: Original referenced an undefined identifier `current`
          // (likely a leftover from a forEach refactor). The change handler
          // must read the index from the actual <select> element `ele`,
          // otherwise this throws a ReferenceError on every native change
          // event in Safari (where MsDropdown.make is called).
          ele.addEventListener("change", () => {
            if (!ele.multiple) {
              ele.msDropdown.selectedIndex = ele.selectedIndex;
            } else {
  ...
  class dd extends HTMLSelectElement {
    ...
    // J1 msDropdown optimizations #1
    // CORRECTNESS: Custom Element constructors cannot accept arguments
    // (https://html.spec.whatwg.org/multipage/custom-elements.html#custom-element-conformance).
    // The original `constructor(element, settings)` signature was misleading
    // -- the parameters were silently ignored because `customElements.define`
    // calls `new dd()` with zero arguments. Removed the dead parameters.
    constructor() {
      super();
    }
    ...
    connectedCallback() {
      // J1 msDropdown optimizations #1
      // CORRECTNESS: The original used `setTimeout(... ,1)` which is
      // race-prone (the wrapper is created off-thread, so any synchronous
      // access to `this.msDropdown` immediately after `appendChild` would
      // miss it) and fires even after the element is disconnected again.
      // Use a microtask via `queueMicrotask` instead -- it runs at the end
      // of the current task, after children are parsed, but before the
      // next render frame, which is exactly what we need.
      // We also keep a reference to the change handler so
      // `disconnectedCallback` can remove it (the original leaked it).
      queueMicrotask(() => {
        ...
        try {
          ...
          this.addEventListener("change", this._msOnChange);
        } catch (e) {
          console.log(e.message);
        }
      });
    ...
    disconnectedCallback() {
      // J1 msDropdown optimizations #1
      // CORRECTNESS: Properly tear down on disconnect to avoid leaking the
      // ddMaker instance and its document-level listeners when the host
      // <select> is removed from the DOM (e.g. SPA route changes).
      if (this._msOnChange) {
        this.removeEventListener("change", this._msOnChange);
        this._msOnChange = null;
      }
      if (this.msDropdown && typeof this.msDropdown.destroy === "function") {
        try {
          this.msDropdown.destroy();
        } catch (e) {
          /* already destroyed */ }
        this.msDropdown = null;
      }
    }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      ...
      // J1 msDropdown optimizations #1
      // CLARITY: Removed an unconditional `console.log('attributeChangedCallback')`
      // that fired on every attribute mutation. Spammed dev consoles with
      // no useful information and was clearly a debug leftover.
    }
  ...
  customElements.define('ms-dropdown', dd, {
    extends: 'select'
  ...
  (function () {
    // J1 msDropdown optimizations #1
    // CLARITY: Original Safari detection chained `.indexOf(...)` checks with
    // mixed `>-1` / `==-1` styles. Rewrote with explicit `includes()` calls
    // and named intermediates so the intent (Safari, but NOT Chrome-on-iOS
    // or Firefox-on-iOS, which both pose as Safari in `vendor`) is obvious.
    ...
    const isSafari = isAppleVendor && !isChromeIOS && !isFirefoxIOS;
    if (isSafari) {

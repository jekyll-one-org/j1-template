/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/speak2me/js/speak2me.js (15)
 # speak2me v.1.15 implementation (based on Articulate.js) for J1 Theme
 #
 # Product/Info:
 # https://jekyll.one
...
var dependency_met_page_ready = setInterval (() => {
  ...
  if (j1CoreFinished && pageVisible) {
    const MODULE_NAME     = 'j1.speak2me.core';
    ...
    // workaround: delay/wait for loading the factory function is finished
    setTimeout(() => {
      ...
      (function($) {
        ...
        const ParseContent = function parseContent (options) {
          ...
          function selectHeadings (contentSelector, headingSelector) {
            ...
            try {
              ...
              isDev && logger.warn('speak2me.core: Could not find', contentSelector);
              return null;
            }
          }
        ...
        // optimization #1: cache DOM elements to avoid repeated queries - cached on first use
        var $content                = null;
        ...
        // optimization #1: store active event listeners for cleanup
        ...
        // default voices for different engines
        // var voiceLanguageGoogleDefault = {
        //   'de-DE': 'Google Deutsch',
        //   'en-GB': 'Google UK English Female',
        ...
        // optimization #1: cache DOM queries
        ...
        function highlightWord(charIndex) {
          ...
          // optimization #1: original code computed adjustedCharIndex
        ...
        function calculateSentenceOffset(sentences, sentenceIndex) {
          var offset = 0;
          // optimization #1: the original space calculation
        ...
        function prepareParagraphToHighlighWords(text) {
          ...
          // optimization #1: removed extraneous double-space
          ...
          // optimization #1: same missing 'g' flag as above
          const words = text
            .replace(/\n+/g, '')
            .replace(/^\s+/, '')
            .split(/\s+/);
        ...
        function normalizeText(text) {
          if (!text) return '';
          // optimization #1: original code discarded the chained
        ...
        function setHighlightParagraph(elementid) {
          ...
          if (isAlreadyHighlighted) {
            isDev && logger.debug(`speak2me.core: setHighlightParagraph called on id current|previous: ${elementid} | ${currentHighlightedElement}`);
          }
          ...
          if ($element) {
            ...
            try {
              ...
              isDev && console.error('speak2me.core: Could not scroll to highlighted element:', error);
            }
            return true;
          }
          return false;
        ...
        function removeParagrapHighlight(dataId) {
          ...
          // optimization #1: document.querySelector() returns either
          // an Element or null — never undefined.  The `!== undefined` check was
          // redundant and misleading about the API contract.
          if ($element !== null) {
            isDev && logger.debug(`speak2me.core: removeParagrapHighlight called on id: ${dataId}`);
            $element.classList.remove('tts-karaoke-highlight-paragraph');
          }

        }

        // optimization #1: improved scan function with better error handling
        function scanPage(options) {
          var line = options.startLine || 0;
          var lines;
          var scanCounter = 0;
          var maxScanIterations = 100; // optimization #1: Prevent infinite loops

          function scanSection() {
            // optimization #1: add maximum iteration check
            if (scanCounter++ > maxScanIterations) {
              isDev && logger.warn('speak2me.core: Page scan exceeded maximum iterations');
              finalizeScan();
              return;
            }

            // optimization #1: calculate once per iteration
            lines = Math.max(
            ...
            // optimization #1: cache jQuery selector
        ...
        // optimization #1: simplified and more efficient
        function extend() {
          var target = {};
          for (var i = 0; i < arguments.length; i++) {
            var source = arguments[i];
            if (source) { // optimization #1: Check for null/undefined
        ...
        // optimization #1: more efficient word counting
        function wordCount(str) {
          if (!str || typeof str !== 'string') return 0; // optimization #1: Input validation
        ...
        function populateVoiceList() {
          // optimization #1: clear existing voices before re-populating
          voices = [];
          
          // optimization #1: repeated string concatenation with +=
          ...
          // optimization #1: systemVoicesText was built by iterating
          ...
          // isDev && logger.debug(systemVoicesText);
          isDev && logger.debug(systemVoicesText);
        }

        // run populateVoiceList
        ...
        // optimization #1: simplified language detection
        if (!currentTranslation) {
          ...
          // optimization #1: use object mapping for language codes
          var languageMap = {
            'en': 'en-GB',
            'ar': 'ar-EG',
            'cs': 'cs-CZ',
        ...
        var methods = {
          ...
          speak: function(options) {
            ...
            // optimization #1: validate options
            if (!myOptions) {
            ...
            // optimization #1: the voiceTag for 'img' was commented
            ...
            // optimization #1: check if speech synthesis is already active
            if (window.speechSynthesis.speaking) {
              isDev && logger.warn('speak2me.core: Speech synthesis already in progress');
              return this;
            }

            // optimization #1: Combine processing with event monitoring in single interval
            ...
            var processSpeech = setInterval(function () {
              ...
              if (scanFinished) {
                ...
                // optimization #1: process all elements in one pass
                try {
                  _this.each(function() {
                    obj = $(this).clone();
                  ...
                  // optimization #1: error handling for DOM processing
                ...
                // optimization #1: remove old event listeners before adding new ones
                // optimization #1: removeEventListener requires the
                ...
                // optimization #1: safe voice selection with fallback
                ...
                // optimization #1: store event listeners for cleanup
                activeEventListeners.onboundary = (event) => {
                  if (event.name === 'word') {
                    ...
                    isDev && logger.debug(`speak2me.core, onboundary: spoken word: '${currentWord}' at startIndex: ${startIndex}`);
                ...
                activeEventListeners.onstart = (event) => {
                  ...
                  if (!isAlredadyHighlighted) {
                    ...
                    if ($paragraph !== undefined && $paragraph !== null) {
                      ...
                      isDev && logger.debug(`speak2me.core, onstart: remove highlight on: ${currentHighlightedElement}`);
                      ...
                      isDev && logger.warn('speak2me.core, onstart: error accessing loose text');

                      // clear all highlights globally
                      $('.tts-karaoke-highlight-paragraph').removeClass('tts-karaoke-highlight-paragraph');
                      isDev && logger.warn('speak2me.core, onstart: clear all highlights globally');
                    }

                    if (speak2meId !== undefined) {
                      isDev && logger.debug(`speak2me.core, onstart: set highlight on: ${speak2meId}`);
                      setHighlightParagraph(speak2meId);                  
                    } else {
                      isDev && logger.warn(`speak2me.core, onstart: could not set highlight on paragraph`);
                    }
                    ...
                    if (currentChunk && currentParagraphSentences.length > 0) {
                      ...
                      if (sentenceIndex >= 0) {
                        ...
                        isDev && logger.debug(`speak2me.core, findSentenceIndexForChunk: sentence ${sentenceIndex} starts at offset ${currentSentenceOffset}`);
                      }
                    }

                  } else {
                    // optimization #1: typo "MOT" → "NOT"
                    isDev && logger.debug(`speak2me.core, onstart: highlight NOT changed on: ${speak2meId}`);
                    ...
                    if (currentChunk && currentParagraphSentences.length > 0) {
                      ...
                      if (sentenceIndex >= 0) {
                        currentSentenceOffset = calculateSentenceOffset(currentParagraphSentences, sentenceIndex);
                        isDev && logger.debug(`speak2me.core, findSentenceIndexForChunk: (same paragraph) sentence ${sentenceIndex} starts at offset ${currentSentenceOffset}`);
                      }
                    }
                  }
                };
            ...
            function splitTextIntoChunks(text) {
              ...
              // optimization #1: chain text cleanup operations
              text = text
                .replace(/^\s+>/gm, '')
                .replaceAll('..', '.')
                .replace(/(\r\n|\n|\r)/gm, '')
              ...
              // optimization #1: single pass cleanup with filter
              chunks = chunks
              ...
              chunks.forEach((chunk, index) => {
                // optimization #1: `var text = chunks[index]` was
              ...
              // optimization #1: more efficient heading parsing
            ...
            function textSlice(text, sliceLength, wordsMin) {
              if (!text || typeof text !== 'string') return undefined; // optimization #1: Validation

              var startSubString = 0;
              var endSubString = startSubString + sliceLength;
              // optimization #1: String.prototype.substr() is
            ...
            // optimization #1: the parameter name `chunks` shadows
            ...
            function processTextChunks(speaker, chunks) {
              ...
              var speechMonitor = setInterval(function () {
                ...
                if (chunkCounter >= chunkCounterMax || userStoppedSpeaking) {
                  ...
                  // optimization #1: Clean up event listeners when done
                  ...
                  if (!wasRunOnce && myOptions.isPaused) {
                    chunkCounter = myOptions.lastChunk || 0; // optimization #1: Add fallback
                    wasRunOnce = true;
                  }

                  // optimization #1: Validate chunk exists before accessing
                  if (chunks[chunkCounter]) {
                    ...
                    isDev && logger.warn('speak2me.core: Invalid chunk at index', chunkCounter);
                    clearInterval(speechMonitor);
                  }
                }
              }, speechMonitorCycle);
            ...
            function processDOMelements(clone, voiceTags, ignoreTags) {
              ...
              // optimization #1: Validate clone exists
              ...
              // optimization #1: Helper function to process blocks with similar structure
              ...
              // optimization #1: Helper for media blocks (audio/video players)
              ...
              // optimization #1: Helper for elements with title in previous sibling
            ...
            // optimization #1: the parameter name `final` is a
            ...
            function cleanDOMelements(final) {
              ...
              // optimization #1: Chain all replacement operations
          ...
          stop: function() {
            if (window.speechSynthesis) {
              ...
              // optimization #1: Clean up event listeners on stop
              // optimization #1: same removeEventListener bug as in
          ...
          ignore: function() {
            ...
            ignoreTagsUser = []; // optimization #1: Direct assignment instead of setting length
          ...
          customize: function() {
            ...
            if (len === 2) {
              if (['img','table','figure'].indexOf(arguments[0]) === -1) {
                isDev && logger.warn("speak2me.core: When customizing, tag must be 'img', 'table', or 'figure'.");
                return this;
              }
            ...
            if (len === 3) {
              if (['q','ol','ul','blockquote'].indexOf(arguments[0]) === -1) {
                isDev && logger.warn("speak2me.core: When customizing, tag must be 'q', 'ol', 'ul' or 'blockquote'.");
                return this;
              }
          ...
          getVoices: function() {
            ...
            // optimization #1: Use filter and forEach for cleaner code
          ...
          setVoice: function() {
            ...
            if (arguments[0] === 'name') {
              ...
              // optimization #1: Use find instead of loop
      ...
      logger.info('\n' + 'factory function initialized successfully');

      endTimeModule = Date.now();
      logger.info('\n' + 'factory initializing time: ' + (endTimeModule-startTimeModule) + 'ms');

...
(function (root, factory) {
  ...
  function doPostOnPlaying(state) {
    ...
    if (_startedFromPlaylist) {
      ...
      // closePlaylist() hides the panel but does not reset the toggle-button
      // UI state (the <span> label, <img> icon, and aria attributes inside
      // #video_player_container).  If the panel was open when playback
      // started, the span still reads "Hide Playlist" after closePlaylist()
      // returns, so the next click of the toggle button tries to close an
      // already-closed panel and appears to do nothing, breaking the toggle.
      // Resetting the toggle UI to its "closed" state here keeps the button
      // in sync with the actual panel state.
  ...
  function _resetPlaylistToggleUI() {
    const wrapper = document.getElementById('video_player_container');
    if (!wrapper) return;
    ...
    handleFileSelected(event) {
      ...
      reader.onload = (e) => {
        try {
          ...
          const videoElement = document.getElementById('video_player_container');
          if (videoElement) {
            scrollToElement(videoElement);
          }
        } catch (err) {

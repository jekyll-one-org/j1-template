...
(function (root, factory) {
  ...
  // claude - change skipAd API to local files #2
  // Replaced YOUTUBE_PATTERNS with VIDEO_URL_PATTERNS.
  // Matches local paths (/assets/...) and remote URLs for MP4, WebM, OGV,
  // OGG, M4V and MOV video files.  A bare filename (no directory) with a
  // recognised extension is also accepted for convenience.
  ...
  const MESSAGES = Object.freeze({
    ...
    // claude - change skipAd API to local files #2
    // Updated error message: references video URL instead of YouTube URL.
    INVALID_URL:        'Invalid video URL or path. Accepted formats: MP4, WebM, OGV.',
  ...
  // claude - change skipAd API to local files #2
  // Renamed skipAdOptions → videoPlayerOptions throughout the module.
  ...
  class PlaylistManager {
              ...
              <div class="modal-body">
                ...
                <div class="rating-dialog">
                  ...
                  <div class="rating-modal-footer">
                    ...
                    <button id="closeRatingBtn" class="rating-modal-btn rating-modal-btn-primary" data-bs-dismiss="modal">Do Nothing</button>
                  </div>
                </div>
              </div>
            </div>
    ...
    openRatingModal(videoId) {
      ...
      // claude - change skipAd API to local files #2
      // Use local poster instead of YouTube CDN thumbnail.
              ...
              <div class="modal-body">
                ...
                <div class="edit-dialog">
                  ...
                  <div class="rating-modal-footer">
                    ...
                    <button id="closeEditBtn" class="rating-modal-btn rating-modal-btn-primary" data-bs-dismiss="modal">Do Nothing</button>
                  </div>
                </div>
              </div>
            </div>
    ...
    openEditModal(videoId) {
      ...
      // claude - change skipAd API to local files #2
      // Video link field defaults to entry.src (native video path/URL)
      // instead of a YouTube watch URL constructed from the videoId.
  ...
  /**
   * embedRunVideo - embed and play a native video file (over videoJS HTML5 tech)
   * @param {string} videoSrc  - full local or remote video URL/path
  ...
  const embedRunVideo = (videoSrc, mode) => {
    ...
    // This matches the derivation used in native.js so playlist entries
    // and player events share the same key.
    const videoId = videoSrc
      ? videoSrc.split('?')[0].split('/').pop().replace(/\.[^.]+$/, '') || videoSrc
      : '';

    const vjsPlayer = createVideoJsPlayer(videoId, videoSrc, {
      title: '',
        ...
        return (event) => {
          ...
          if (vjsStateEventNameMap[state] === 'paused' || vjsStateEventNameMap[state] === 'ended') {
            try {
              ...
              // claude - change skipAd API to local files #2
              // videoId is now sourced from player.videoData (native) instead
              // of player.ytVideoData (YouTube).
              ...
              if (vjsStateEventNameMap[state] === 'ended' && loopConfigEnabled && playlistManager._loopEnabled) {
                ...
                if (nextId) {
                  ...
                  // claude - change skipAd API to local files #2
                  // Resolve src from playlist for the next item so embedRunVideo
                  // receives a proper file URL rather than a bare videoId string.
      ...
      // claude - change skipAd API to local files #2
      // onReady now wires up to the 'videoDataResolved' custom DOM event
      // dispatched by native.js instead of reading from the YouTube tech's
      // getVideoData() / tech.videoData() methods.
      onReady: (player) => {
        isDev && logger.info('\n'+ 'vjs player initialized and ready');

        // claude - change skipAd API to local files #2
        // applyVideoData stores metadata on player.videoData (native) instead
        // of player.ytVideoData (YouTube).  The 'videoTitleResolved' event
        // replaces the former 'ytVideoTitleResolved' event.
        const applyVideoData = (videoData) => {
          ...
          // claude - change skipAd API to local files #2
          // Dispatch 'videoTitleResolved' instead of the former 'ytVideoTitleResolved'.
        ...
        // claude - change skipAd API to local files #2
        // Listen for the 'videoDataResolved' DOM event dispatched by native.js
        // (nativePlayer plugin) instead of polling the YouTube tech.
        // The listener is removed after the first successful resolve to avoid
        // accumulating duplicate handlers across player recreations.
        ...
        // playlistManager, fixed video duration
        player.on('durationchange', () => {
          ...
          // claude - change skipAd API to local files #2
          // Reference changed from skipAd.playlistManager to videoPlayer.playlistManager.
          const seconds      = videoPlayer.playlistManager._parseDuration(durationText);

          if (seconds > 0) {
            // claude - change skipAd API to local files #2
            // Use player.videoData.videoId (native) instead of
            // player.ytVideoData.video_id (YouTube).
            const currentVideoId = player.videoData && player.videoData.videoId
              ? player.videoData.videoId
              : videoId;
        ...
        if (videoPlayerOptions.videoJS.autoStart) {
          ...
          // claude - change skipAd API to local files #2
          // Autoplay option is now read from videoPlayerOptions.videoJS.players.native
          // instead of videoPlayerOptions.videoJS.players.youtube.
          if (videoPlayerOptions.videoJS.players.native && videoPlayerOptions.videoJS.players.native.autoplay) {
            if (playerMode === 'pause') {
   ...
   * claude - change skipAd API to local files #2
   * Removed all YouTube-specific getVideoData() / ytPlayer calls.
   * Video metadata is now read from player.videoData (set by the
   * 'videoDataResolved' event handler in onReady).
  ...
  function doPostOnPlaying(state) {
    ...
    // claude - change skipAd API to local files #2
    // Use player.videoData (native) instead of player.ytVideoData (YouTube).
    // Dispatch 'videoPlayingStarted' with the native videoId.
    ...
    // claude - change skipAd API to local files #2
    // Read metadata from player.videoData populated by the nativePlayer plugin
    // via the 'videoDataResolved' DOM event (see onReady above).
    // No YouTube IFrame API (ytPlayer.getVideoData()) involved.
    const vd = player.videoData || {};
    const vid = vd.videoId || '';

    // update the title display element
    if (textEl) {
    ...
    // claude - change skipAd API to local files #2
    // Construct playlist entry from native video data.
    // 'src' stores the full video URL; 'videoLink' mirrors src for consistency.
    const media = {
      src:          vd.src      || '',
      poster:       vd.poster   || '',
      videoId:      vid,
   ...
   * createVideoJsPlayer
   * claude - change skipAd API to local files #2
   * Replaced YouTube-tech player creation with native HTML5 tech.
   ...
   *
   * @param {string} videoId  - derived filename-without-extension key
   * @param {string} videoSrc - full local or remote video URL/path
   ...
   */
  function createVideoJsPlayer(videoId, videoSrc, options = {}) {
    ...
    const videoConfig = {
      fluid:     true,
      // claude - change skipAd API to local files #2
      // techOrder changed from ['youtube', 'html5'] to ['html5'] — native only.
      techOrder: ['html5'],
      sources: [{
        type: srcType,
        src:  videoSrc
   ...
   * inputWrapperHandler
   * claude - change skipAd API to local files #2
   * Replaced YouTube URL extraction with native video URL/path extraction.
   * Accepts local paths (/assets/video/...) and remote MP4/WebM/OGV URLs.
   * The YOUTUBE_PATTERNS constant is no longer referenced; VIDEO_URL_PATTERNS
   * is used instead.  extractVideoId() is renamed to extractVideoSrc() but
   * the class keeps the same public interface for compatibility.
   */
  class inputWrapperHandler {
    ...
    processUrl() {
      ...
      // claude - change skipAd API to local files #2
      // extractVideoSrc replaces extractVideoId.
      // Returns the raw URL/path if it matches any known video pattern.

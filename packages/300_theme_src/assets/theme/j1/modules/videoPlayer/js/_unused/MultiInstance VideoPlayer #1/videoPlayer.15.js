/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/videoPlayer/js/videoPlayer.js (15)
...
/* Version 3.1.15 for J1 Template */
...
(function (root, factory) {
  ...
  class PlaylistManager {
    ...
    // After the MultiInstance #2 HTML refactor all in-container element IDs are
    // namespaced as `${baseId}_${containerId}` (e.g. "serverPlaylistSelect_player_1").
    // The lookup order is:
    //   1. bare   `#${baseId}`            inside container (pre-#2 HTML compatibility)
    //   2. namespaced `#${baseId}_${containerId}` inside container (#2+ HTML)
    //   3. document.getElementById with namespaced id (global fallback for #2+ HTML)
    //   4. document.getElementById with bare id (legacy global fallback)
    //
    // claude - MultiInstance VideoPlayer #4
    // Lazy containerEl resolution: createInstance() may be called before the
    // XHR-injected HTML is in the DOM, leaving inst.containerEl null.  On each
    // _scopedId call, if containerEl is still null, re-attempt getElementById so
    // that once the HTML arrives the very next lookup succeeds and the element is
    // cached back onto the instance.
    _scopedId(baseId) {
      ...
      if (inst) {
        ...
        if (inst.containerEl) {
          ...
          if (el) return el;
          // claude - MultiInstance VideoPlayer #3: try namespaced ID
          const namespacedId = `${baseId}_${inst.containerId}`;
          const elNs = inst.containerEl.querySelector(`#${namespacedId}`);
          if (elNs) return elNs;
          // global fallback with namespaced id
  ...
  function createVideoJsPlayer(inst, videoId, videoSrc, isYouTube, options = {}) {
    ...
    // claude - MultiInstance VideoPlayer #3: also try namespaced ID for post-#2 HTML.
    const _overlayNsId = inst.containerId ? `emptyPlayerOverlay_${inst.containerId}` : null;
    const overlayExists = container.querySelector('#emptyPlayerOverlay')
      || (_overlayNsId ? container.querySelector(`#${_overlayNsId}`) : null)
  ...
  class inputWrapperHandler {
    ...
    _scopedEl(id) {
      ...
      if (inst) {
        ...
        if (inst.containerEl) {
          // claude - MultiInstance VideoPlayer #3: try bare id first (pre-#2 HTML),
          // then namespaced id (post-#2 HTML), then global namespaced fallback.
    ...
    handleFileSelected(event) {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (container) {
            const _ovNsId = inst.containerId ? `emptyPlayerOverlay_${inst.containerId}` : null;
            const overlayExists = container.querySelector('#emptyPlayerOverlay')
              || (_ovNsId ? container.querySelector(`#${_ovNsId}`) : null)
              || container.querySelector('.empty-player-overlay');
            if (!overlayExists) {
    ...
    async handleLoadFromServer() {
      ...
      if (inst && inst.containerEl) {
        ...
        const overlayExists = inst.containerEl.querySelector('#emptyPlayerOverlay')
          || (_ovNsId ? inst.containerEl.querySelector(`#${_ovNsId}`) : null)
  ...
  class playlistModeSwitchHandler {
    ...
    _scopedEl(id) {
      ...
      if (inst) {
        ...
        if (inst.containerEl) {
          // claude - MultiInstance VideoPlayer #3: try bare id first, then namespaced.
          const el = inst.containerEl.querySelector(`#${id}`);
          if (el) return el;
          const namespacedId = `${id}_${inst.containerId}`;
          const elNs = inst.containerEl.querySelector(`#${namespacedId}`);

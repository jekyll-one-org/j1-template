...
var progress;
    ...
    {% for player in amplitude_options.players %}{% if player.enabled and player.source == 'video' %}
      ...
      {% else %}
        ...
        if (playerExistsInPage) { 
          ...
          // save YT player data for later use (e.g. events)
          // j1.adapter.amplitude.data.ytPlayers[{{player.id}}].player = ytPlayer_{{player.id};
        ...
        function {{player.id}}OnPlayerStateChange(event) {
          ...
          if (event.data === YT_PLAYER_STATE.CUED) {
            ...
            logger.debug('\n' + 'video at track|state: ' + trackID + '|' + YT_PLAYER_STATE_NAMES[event.data]);
            return;
          }
          ...
          if (event.data === YT_PLAYER_STATE.PLAYING) {
            ...
            // save YT player data for later use (e.g. events)
            j1.adapter.amplitude.data.ytPlayers[player].activeIndex = songIndex;
            ...
            if (songStartSec) {
              ...
              if (currentVideoTime <= songStartSec) {
                ...
                // logger.debug('\n' + 'seek video at track|second: ' + trackID + '|' + songStartSec);
            ...
            if (songEndSec) {
              ...
              var checkOnVideoEnd = setInterval(function() {
                ...
                if (currentVideoTime >= songEndSec) {
                  ...
                  logger.debug('\n' + 'load next video at track|ID: ' + trackID + '|' + ytpVideoID);
                  logger.debug('\n' + 'load next video at track|timestamp: ' + trackID + '|' + tsEndSec);
                  nextVideo(ytpVideoID);
  ...
  function updateProgressBarsYTP() {
    ...
    var activePlaylist  = j1.adapter.amplitude.data.ytpGlobals['activePlaylist'];
  ...
  // ---------------------------------------------------------------------------
  // ytpGetCurrentPlaylist
  //
  // ???????
  // ---------------------------------------------------------------------------
  function ytpGetCurrentPlaylist(player, type='large') {
    var playerIframeID, playerID, playerPlaylist;

    playerIframeID  = player.g.id;
    playerID        = playerIframeID.replace(/iframe_/g, '');
    playerPlaylist  = playerID.replace(/_large/g, '');

    return playerPlaylist;
  } // END ytpGetCurrentPlaylist
  ...
  function mimikYTPlayerUiEventsForAJS(ytPlayerID) {
        ...
        for (var i=0; i<largePlayerPlayPauseButton.length; i++) {
          ...
          if (classString.includes(ytPlayerID)) {
            largePlayerPlayPauseButton[i].addEventListener('click', function(event) {
              var playerIframeID, currentPlayerID, currentPlaylist;
              var playlist          = this.getAttribute("data-amplitude-playlist");

              playerIframeID    = ytPlayer.g.id;
              currentPlayerID   = playerIframeID.replace(/iframe_/g, '');
              currentPlaylist   = currentPlayerID.replace(/_large/g, '');
              playerID          = ytPlayerID;
              ...
              var currenVideoID     = ytPlayer.options.videoId;
              ...
              //var playerState     = activePlayer.getPlayerState();
              ...
              // change video
              // if (activeVideoID !== currenVideoID) {
              //   ytPlayer.loadVideoById(activeVideoID);
              //   // return;
              // }

              // When a video is cued and ready to play
    ...
    for (var i=0; i<largetPlayerSongContainer.length; i++) {
      ...
      if (classString.includes(ytPlayerID)) {
        largetPlayerSongContainer[i].addEventListener('click', function(event) {
          var playlist, playerID, ytPlayer, playerState, activeIndex, songs, songIndex,
              songMetaData, songURL, trackID, songStart, songStartSec, songName,
              coverImage, changedAudio, selector, playerCurrentTime, ytpVideoID;
    ...
    if (volumeMutes.length) {
      for (var i=0; i<volumeMutes.length; i++) {
        var volumeMute = volumeMutes[i];
        var playerId   = progressId.split('large_player_progress_')[1];
        var ytPlayer   = j1.adapter.amplitude.data.ytPlayers[playerId].player;

        // save YT player data for later use (e.g. events)
        j1.adapter.amplitude.data.ytPlayers[playerId].volumeMute = volumeMute;

...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/amplitudejs/js/plugins/tech/ytp.22.js
...
var messageLoggedOnce = false;
  ...
  function nextVideo() {
    ...
    var ytPlayerID  = playlist + '_large';
      ...
      {% else %}
        ...
        function {{player.id}}OnPlayerStateChange(event) {
          ...
          if (event.data === YT_PLAYER_STATE.PLAYING) {
            ...
            if (audioFade && songStartSec) {
              var tStartSec        = seconds2timestamp(songStartSec);
              var videoCurrentTime = ytPlayer.getCurrentTime();

              // set configured player volume
              // ytPlayer.setVolume(0);

              // check|process video for configured START position
              if (videoCurrentTime <= songStartSec) {

                // seek video to START position
                ytpSeekTo(ytPlayer, songStartSec, true);
                // logger.debug('\n' + 'seek video at track|second: ' + trackID + '|' + songStartSec);
                logger.debug('\n' + 'start video at track|timestamp: ' + trackID + '|' + tStartSec);
          ...
          if (classString.includes(ytPlayerID)) {
            largePlayerSkipForwardButtons[i].addEventListener('click', function(event)  {
              ...
              if (playerState === YT_PLAYER_STATE.PLAYING || playerState === YT_PLAYER_STATE.PAUSED) {
                ...
                ytpSeekTo(ytPlayer, currentTime + skipOffset, true);
        ...
        for (var i=0; i<largePlayerNextButton.length; i++) {
          ...
          if (classString.includes(ytPlayerID)) {
            largePlayerNextButton[i].addEventListener('click', function(event) {
              var playlist, playerID, songIndex, songs, songMetaData,
              ...
              ytPlayer.loadVideoById(ytpVideoID);
              // ytpLoadVideoById(ytPlayer, ytpVideoID, ytpBufferQuote )             
    ...
    for (var i=0; i<largetPlayerSongContainer.length; i++) {
      ...
      if (classString.includes(ytPlayerID)) {
        largetPlayerSongContainer[i].addEventListener('click', function(event)  {
          var playlist, playerID, playerState, songs, songIndex, ytpVideoID;

...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/amplitudejs/js/plugins/tech/ytp.js (36)
...
"use strict";
  ...
  function doNothingOnStateChange(state) {
    if (state > 0) {
      isDev && logger.debug(`DO NOTHING on StateChange for state: ${YT_PLAYER_STATE_NAMES[state]}`);
    } else {
      isDev && logger.debug(`DO NOTHING on StateChange for state: ${YT_PLAYER_STATE_NAMES[6]}`);
    }

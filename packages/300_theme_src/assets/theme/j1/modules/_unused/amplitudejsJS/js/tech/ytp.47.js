...
{% comment %}
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/amplitudejs/js/tech/ytp.js (47)
...
"use strict";
  ...
  const YT_PLAYER_ERROR_NAMES = {
    ...
    100:        "video not found",
    153:        "embedder identity missing (origin|referrer)"
  };
...
//  previousPlayer  = j1.adapter.amplitude.data.ytPlayers[playerID].player
  ...
  function initYtAPI() {
    ...
    // Fix Amplitude plugin #4
    // Load the YT IFrame API from the CANONICAL host www.youtube.com — the
    // SAME source the VideoJS YouTube tech uses for the J1 MultiPlayer
    // ('//www.youtube.com/iframe_api'). The bare host 'youtube.com' answers
    // with a redirect to 'www.youtube.com'; the extra hop is unnecessary
    // and MIXES two API origins on pages that load BOTH modules.
    // Original (deprecated, preserved for reference):
  ...
  function loadVideo(currentPlaylist, currentIndex) {
    ...
    if (songIndex <= songs.length - 1) {
      ...
      // Fix Amplitude plugin #4
      // RESET the global API error flag BEFORE the new load attempt. The
      // flag ytpGlobals.ytApiError is a SINGLE global value: once ANY video
      // failed (e.g. one mistyped video ID in the playlist), the flag stayed
      // set and EVERY later play|next|previous request was refused by the
      // error gates — the player was broken from then on, while the J1
      // MultiPlayer recovers per video. A load of a NEW video starts with a
      // clean error state; if THIS video fails as well, onPlayerErrors sets
      // the flag again and the gates apply to the CURRENT video only.
      //
        ...
        {% for player in amplitude_options.players %}{% if player.enabled and player.source == 'video' %}
          ...
          {% else %}
            ...
            if (playerExistsInPage) { 
              ...
              var privacyEnhancedHost = (ytpPrivacy) ? '//youtube-nocookie.com' : '//youtube.com';
  ...
  function configureYtPlayer(playerConfig) {
    ...
    if (playerExistsInPage) {
      ...
      // Fix Amplitude plugin #4
      // (1) The per-player key is configured at PLAYER level in
      // amplitude_control.yml ('privacy_enhanced', see e.g. player
      // 'emancipator_yt_large') but was ONLY looked up under
      // 'yt_player.privacy_enhanced' — the user setting was silently
      // ignored and the default always won. The player-level key is now
      // honored FIRST; the yt_player subtree key stays supported.
      // (2) The classic (non-privacy) host 'https://youtube.com' is NOT a
      // valid value for the IFrame API 'host' option; only
      // 'https://www.youtube.com' and '//youtube-nocookie.com'
      // are supported. The bare host forces a redirect INSIDE the iframe
      // that breaks the embedder (origin) check and yields the errors
      // 101|150 'video not allowed' for videos that ARE embeddable. The
      // VideoJS YouTube tech of the J1 MultiPlayer never sets an invalid
      // host (it sets host ONLY for the nocookie case) — one reason the
      // SAME videos play there without errors.
      // Original (deprecated, preserved for reference):
      // var ytpPrivacy          = ytpGetValue(playerConfig, 'yt_player.privacy_enhanced', ytpDefault('player.yt_player.privacy_enhanced', true));
      // var privacyEnhancedHost = (ytpPrivacy) ? '//youtube-nocookie.com' : 'https://youtube.com';
      //
      var ytpPrivacy          = ytpGetValue(playerConfig, 'privacy_enhanced', ytpGetValue(playerConfig, 'yt_player.privacy_enhanced', ytpDefault('player.yt_player.privacy_enhanced', true)));
      var privacyEnhancedHost = (ytpPrivacy) ? '//youtube-nocookie.com' : '//youtube.com';
      ...
      ytPlayer = new YT.Player('iframe_' + playerId, {
        ...
        playerVars: {
          ...
          loop:             ytpLoop,
          // Fix Amplitude plugin #4
          // Explicitly identify the EMBEDDING page to the YT API. The ytp
          // iframe is created INVISIBLE (container display:none) and — when
          // privacy_enhanced is active — served from the STRICTER host
          // www.youtube-nocookie.com. Without the 'origin' parameter, the
          // API validates the embedder via the HTTP Referer alone; for the
          // hidden iframe this check can fail (dev hosts like 'localhost',
          // referrer policies, PiP contexts), and label-claimed music
          // content is then rejected with 101|150 'video not allowed'
          // although the video IS embeddable. The VideoJS YouTube tech of
          // the J1 MultiPlayer sets 'origin' since the fix
          // 'fix pip on youtube' — the SECOND reason the SAME
          // videos play there without errors. 'widget_referrer' passes the
          // full page URL for the same embed attribution.
    ...
    function onPlayerStateChange(event) {
      ...
      switch(event.data) {
        ...
        case YT_PLAYER_STATE.PLAYING:
          // Fix Amplitude plugin #4
          // A video that reaches state PLAYING has passed all embed checks:
          // wipe any STALE error left from a previous video so the global
          // flag always describes the CURRENT video (see loadVideo).
          ytpHostData().ytpGlobals['ytApiError'] = 0;
          ytpHostModuleYtp().apiError            = 0;
          processOnStateChangePlaying(event, playlist, songIndex);
          break;
        ...
        for (var i=0; i<largePlayerNextButton.length; i++) {
          ...
          if (classString.includes(ytPlayerID)) {
            largePlayerNextButton[i].addEventListener('click', function(event) {
              ...
              // Fix Amplitude plugin #4
              // The NEXT button is the natural RECOVERY path when the
              // current video failed (e.g. 'video not allowed' or a
              // mistyped video ID): SKIP AWAY from the broken track. The
              // original gate refused the click as long as the global error
              // flag was set — since the flag was never reset, ONE failed
              // video disabled the navigation forever. The gate now logs a
              // WARNING, CLEARS the flag and PROCEEDS to the next track;
              // loadVideo() starts the new attempt with a clean error state.
              // Original (deprecated, preserved for reference):
              // // Fix Amplitude plugin #2
              // // Original (deprecated, preserved for reference):
              // // if (j1.adapter.amplitude.data.ytpGlobals.ytApiError > 0) {
              ...
              if (ytpHostData().ytpGlobals.ytApiError > 0) {
                var trackID = songIndex + 1;
                logger.warn('\n' + `SKIP failed playlist|trackID: ${playlist}|${trackID} on API error '${YT_PLAYER_ERROR_NAMES[ytpHostData().ytpGlobals.ytApiError]}'`);

                // clear the stale error and proceed to the NEXT track
                ytpHostData().ytpGlobals['ytApiError'] = 0;
                ytpHostModuleYtp().apiError            = 0;
              }
      ...
      for (var i=0; i<largePlayePreviousButton.length; i++) {
        ...
        if (classString.includes(ytPlayerID)) {
          largePlayePreviousButton[i].addEventListener('click', function(event) {
            ...
            // Fix Amplitude plugin #4
            // Same recovery semantics as the NEXT button above: the
            // PREVIOUS button must stay usable to navigate AWAY from a
            // failed track. Log a warning, clear the stale error, proceed.
            // Original (deprecated, preserved for reference):
            // // Fix Amplitude plugin #2
            // // Original (deprecated, preserved for reference):
            // // if (j1.adapter.amplitude.data.ytpGlobals.ytApiError > 0) {
            ...
            if (ytpHostData().ytpGlobals.ytApiError > 0) {
              ...
              // clear the stale error and proceed to the PREVIOUS track
              ytpHostData().ytpGlobals['ytApiError'] = 0;
              ytpHostModuleYtp().apiError            = 0;
            }

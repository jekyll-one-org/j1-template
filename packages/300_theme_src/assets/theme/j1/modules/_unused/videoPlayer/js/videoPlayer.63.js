/*
 ...
 # Version 3.1.63 for J1 Template
 #
 # Product/Info:
 # https://jekyll.one
 #
...
(function (root, factory) {
  ...
  // claude - Modify J1 VideoPlayer expiry date #1
  // ---------------------------------------------------------------------------
  ...
  function _expiryHintText(expiryDate) {
    ...
    if (state === EXPIRY_STATE.EXPIRED) {
      return (daysLeft === 0)
        ? 'Access EXPIRED today - playback disabled'
        : `Access EXPIRED ${Math.abs(daysLeft)} day(s) ago - playback disabled`;
    }
    return `Access ends in ${daysLeft} day(s)`;
  }
  ...
  function _resolveNativePosterConfig() {
    // claude - Modify J1 VideoPlayer #49
  ...
  const mapVideoPlayerPlaylist = Object.freeze({
    ...
    issueDate:    'issueDate',
    // claude - Modify J1 VideoPlayer expiry date #1
    // Carry the optional expiry date through into the videojs-playlist item so
    // downstream consumers of the converted item list (and any adapter reading
    // player.playlist()) see the same value the J1 record holds.
  ...
  // to videoPlayer(id, options) at the "Modify J1 VideoPlayer #48" site MUST be
  ...
  // claude - Modify J1 VideoPlayer for export #2
  // ---------------------------------------------------------------------------
  ...
  class PlaylistManager {
     ...
     * @param {string}  [entry.expiryDate]    - claude - Modify J1 VideoPlayer expiry date #1
     *                                          optional access expiry date (ISO string or
     *                                          free text). Empty string = unlimited (default)
    ...
    addEntry(entry) {
      ...
      const record = {
        ...
        // claude - Modify J1 VideoPlayer expiry date #1
        // Optional access expiry. Defaults to EXPIRY_UNLIMITED ('') so an
        // entry created without the field stays playable forever.
    ...
    createEntry(entry) {
      ...
      const record = {
        ...
        issueDate:    this._normalizeIssueDate(entry.issueDate || ''),
        // claude - Modify J1 VideoPlayer expiry date #1
        // Optional access expiry, seeded at early creation time. Defaults to
        // EXPIRY_UNLIMITED ('') - unlimited access unless explicitly set.
        expiryDate:   this._normalizeIssueDate(entry.expiryDate || EXPIRY_UNLIMITED),
        episode:      entry.episode       || 0,
        lastPosition: entry.lastPosition  || 0,
        poster:       entry.poster        || '',
    ...
    updateEntryFields(videoId, fields) {
      ...
      // claude - Modify J1 VideoPlayer expiry date #1
      // An EMPTY submitted value is written through deliberately: clearing the
      // input in the edit dialog is the documented way to reset an entry back
      // to unlimited access.
    ...
    playEntry(videoId) {
      ...
      if (_isEntryExpired(expiryEntry)) {
        logger.warn('\n' + `playlistmanager: playback DISABLED for videoId: ${videoId} - ` +
                           `access expired on ${expiryEntry.expiryDate} (${_expiryHintText(expiryEntry.expiryDate)})`);
        return;
      }
    ...
    embedRunVideo(videoId, mode) {
      ...
      if (_isEntryExpired(entry)) {
        logger.warn('\n' + `playlistManager: embed DISABLED for videoId: ${videoId} - ` +
                           `access expired on ${entry.expiryDate} (${_expiryHintText(entry.expiryDate)})`);
        return;
      }
    ...
    _initEditModalEvents() {
      ...
      clearBtn.addEventListener('click', () => {
        ...
        // claude - Modify J1 VideoPlayer expiry date #1
        // Clearing resets the entry to UNLIMITED access; the colour/hint sync
        // below returns the field to its neutral appearance immediately.
      ...
      saveBtn.addEventListener('click', () => {
        if (this._editModalVideoId) {
          ...
          const fields = {
            ...
            // claude - Modify J1 VideoPlayer expiry date #1
            // An empty string is a meaningful value here: it resets the entry
            // to unlimited access (see updateEntryFields()).

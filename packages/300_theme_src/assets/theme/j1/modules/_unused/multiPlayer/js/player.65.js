/*
 ...
 # Version 3.1.65 for J1 Template
 #
 # Product/Info:
 # https://jekyll.one
 #
...
(function (root, factory) {
  ...
  // Modify J1 VideoPlayer expiry date #2
  // ===========================================================================
    ...
    const vjsPlayer = createVideoJsPlayer(videoId, videoSrc, isYouTube, {
      ...
      onReady: (player) => {
        ...
        if (videoPlayerOptions.videoJS.autoStart) {
          ...
          // Fix multiPlayer disable piZoomButtons #1
          // -------------------------------------------------------------------
          // EXTENDED enablement rule for the ZoomButtons plugin.
          //
          // The plugin only makes sense on a VISIBLE video surface. It was
          // already suppressed for YouTube sources (jadams, condition kept
          // unchanged as the first guard). Two further cases now suppress it
          // as well:
          //
          //   1. NATIVE .mp3 SOURCES — an audio file renders no video surface;
          //      zoom/rotate controls on the empty (poster) stage are dead UI.
          //      Classification mirrors _getEntryDownloadSource() rule 3
          //      (query string and fragment stripped BEFORE the extension
          //      test, so '.../track.mp3?token=x#t=30' still resolves to mp3),
          //      applied to the source the player actually plays (videoSrc).
          //      A defensive second test consults the persisted playlist
          //      record via _getEntryDownloadSource(), covering records whose
          //      stored src carries the extension while the passed-in URL
          //      does not — the two classifiers can never disagree on the
          //      same record.
          //
          //   2. AUDIO ONLY active (per requirement: "when enableAudioOn is
          //      set") — resolved through _resolveAudioOnlyActive(), i.e. the
          //      SAME precedence chain as the title-bar switch (localStorage
          //      user toggle <- YAML audio_only <- false). While the mode is
          //      active the playback surface shows a static poster
          //      (audioPosterMode, audio only Fix #2) or nothing at all, so
          //      the zoom UI is suppressed for EVERY source of this player
          //      for a consistent title-bar/control-bar state — not only for
          //      the YouTube entries the mode technically applies to.
          //
          // Additive only: the original enablement line is preserved below as
          // a deprecated reference; no original line was removed.
          // -------------------------------------------------------------------
          //
          const zbSourcePath  = String(videoSrc || '').split('#')[0].split('?')[0].toLowerCase();
          const zbEntry       = playlistManager.getEntry(videoId);
          const zbEntrySource = _getEntryDownloadSource(zbEntry);
          const zbIsMp3Source = zbSourcePath.slice(-4) === '.mp3' || !!(zbEntrySource && zbEntrySource.ext === 'mp3');
          const zbAudioOnly   = _resolveAudioOnlyActive();

          // Fix multiPlayer disable piZoomButtons #1
          if ((zbIsMp3Source || zbAudioOnly) && piZoomButtons.enabled) {
            isDev && logger.debug('\n' + 'piZoomButtons: plugin DISABLED for this player - ' +
                                         `mp3 source: ${zbIsMp3Source}, audio only active: ${zbAudioOnly}`);
          }

          if (piZoomButtons.enabled && !isYouTube && !zbIsMp3Source && !zbAudioOnly) {
            // piZoomButtons

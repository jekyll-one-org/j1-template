...
//    history.push({ role: 'user', content: apiContent });
    ...
    function appendMessage(role, content) {
      ...
      // Wire the click delegation that turns each avatar into a
      // delete-pair button. wireAvatarDeletion() is idempotent (guarded
      // by a dataset flag), so calling it on every appendMessage() is
      // free and ensures the handler is attached regardless of whether
      // loadHistory() ran first.
      wireAvatarDeletion(container);
      ...
      // claude - J1 claudeAI modifications #3
      // Make the avatar behave (and announce itself) as a button so the
      // hover-as-delete affordance is discoverable for sighted users
      // and operable for keyboard / screen-reader users.
      avatar.setAttribute('title',    'Click to delete this Q & A from the conversation');
      avatar.setAttribute('role',     'button');
      avatar.setAttribute('tabindex', '0');
      avatar.setAttribute('aria-label',
        role === 'assistant'
          ? 'Delete this answer and the question that produced it'
    ...
    function appendTyping() {
      ...
      const avatar = document.createElement('div');
      // claude - J1 claudeAI modifications #3
      // The transient typing indicator does not correspond to a stored
      // history entry yet. Add 'is-streaming' so the CSS suppresses the
      // delete-button hover overlay and the click handler ignores it.
      avatar.className = 'chat-msg-avatar is-streaming';
    ...
    // -------------------------------------------------------------------------
    // claude - J1 claudeAI modifications #3
    // Selective deletion of a single chat entry.
    //
    // realMessages(container)
    //   Returns the .chat-msg elements that correspond to real history
    //   entries — i.e. excludes the transient typing indicator. The
    //   returned array index aligns with the linear `history` array
    //   (even = user, odd = assistant).
    //
    // deletePair(domIndex)
    //   Removes the user/assistant pair containing the message at
    //   domIndex from BOTH `history` (persisted via Storage) and the
    //   DOM. If the message at domIndex has no assistant counterpart
    //   (lonely trailing user message), only that single entry is
    //   removed. A native confirm() dialog guards against accidents.
    //
    // wireAvatarDeletion(container)
    //   Idempotently attaches click + keyboard delegation to
    //   #chatMessages so a click (or Enter/Space) on any avatar
    //   triggers deletePair() with the correct index.
    // -------------------------------------------------------------------------
    function realMessages(container) {
      if (!container) return [];
      return Array.from(container.querySelectorAll('.chat-msg'))
        .filter((el) =>
          el.id !== 'typingMsg' && !el.querySelector('.typing-indicator')
        );
    }

    async function deletePair(domIndex) {
      if (!Array.isArray(history) || history.length === 0) return false;

      // claude - J1 claudeAI modifications #3
      // While a request is in flight, the assistant reply is not yet in
      // `history` but will be pushed when the response arrives. Allowing
      // deletion now could orphan that pending entry. Refuse and prompt
      // the user to wait — they can still use Clear Conversation if they
      // really want to wipe everything.
      if (isStreaming) {
        Toast.show('Please wait for the current answer to finish before deleting.', 'info', 3000);
        return false;
      }

      // Snap to the pair head: even idx = user, odd = assistant.
      const pairStart = (domIndex % 2 === 0) ? domIndex : domIndex - 1;
      if (pairStart < 0 || pairStart >= history.length) return false;

      // Two entries unless we're at the tail with a lonely user message.
      const removeCount = Math.min(2, history.length - pairStart);

      // Confirm the deletion
      // -----------------------------------------------------------------------
      // const promptMsg = (removeCount === 2)
      //   ? 'Delete this question and its answer from the conversation?'
      //   : 'Delete this question from the conversation?';
      // if (!window.confirm(promptMsg)) return false;

      // Splice in-memory history first (gives instant UI feedback).
      history.splice(pairStart, removeCount);

      // Remove the corresponding DOM nodes.
      const container = document.getElementById('chatMessages');
      if (container) {
        const msgs = realMessages(container);
        for (let i = 0; i < removeCount; i++) {
          const el = msgs[pairStart + i];
          if (el && el.parentNode) el.parentNode.removeChild(el);
        }

        // Restore the welcome screen when the conversation is empty.
        if (history.length === 0 && !document.getElementById('welcomeScreen')) {
          container.innerHTML = `
      ...
      // Persist asynchronously. We resolve the storage key the same way
      // clear() does, so a custom chatHistoryKey from YAML still works.
      try {
        const settings   = (typeof AgentUI !== 'undefined' && AgentUI.getSettings)
          ? AgentUI.getSettings()
          : null;
        const storageKey = (settings && settings.chatHistoryKey)
          ? settings.chatHistoryKey
          : HISTORY_KEY;
        await Storage.set(storageKey, JSON.stringify(history));
      } catch (e) {
        consoleLog('WARN', MODULE_NAME, 'deletePair: failed to persist history: ' + e);
      }

      // Refresh the table-of-contents (assistant bubbles contribute headings).
      if (typeof tocbot !== 'undefined' && typeof tocbot.refresh === 'function') {
        try { tocbot.refresh(); } catch (e) { /* tocbot is best-effort */ }
      }

      consoleLog('INFO', MODULE_NAME,
        `deletePair: removed history[${pairStart}..${pairStart + removeCount - 1}], remaining=${history.length}`);
      return true;
    }

    function wireAvatarDeletion(container) {
      if (!container || container.dataset.deletePairWired === 'true') return;
      container.dataset.deletePairWired = 'true';

      // Mouse / touch
      container.addEventListener('click', (event) => {
        const avatar = event.target.closest('.chat-msg-avatar');
        if (!avatar || !container.contains(avatar)) return;
        if (avatar.classList.contains('is-streaming')) return;

        const msgEl = avatar.closest('.chat-msg');
        if (!msgEl || msgEl.id === 'typingMsg') return;
        if (msgEl.querySelector('.typing-indicator')) return;

        const idx = realMessages(container).indexOf(msgEl);
        if (idx < 0) return;

        // Fire-and-forget: deletePair is async because of Storage.set,
        // but the visible DOM update happens synchronously inside it.
        deletePair(idx);
      });

      // Keyboard (Enter / Space) — avatars carry tabindex="0".
      container.addEventListener('keydown', (event) => {
        if (event.key !== 'Enter' && event.key !== ' ') return;
        const avatar = event.target.closest && event.target.closest('.chat-msg-avatar');
        if (!avatar || !container.contains(avatar)) return;
        event.preventDefault();
        avatar.click();
      });
  ...
  const AgentUI = (() => {
    ...
    // claude - J1 claudeAI modifications #3
    // Public entry point that mirrors the existing clearChat() naming.
    // Delegates to AgentChat.deletePair() which owns the history array
    // and the persistence layer. Exposed as claudeAi.AgentUI.deleteMessagePair
    // so it can be invoked from inline onclick handlers if the renderer
    // is later changed to emit them directly, or from other J1 modules
    // that want to programmatically prune the conversation.
    async function deleteMessagePair(domIndex) {
      return AgentChat.deletePair(domIndex);
    }

/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/claudeAi/js/claudeAi.js (1)
...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    // Claude - class claudeAiHandler fixes: register as 'claudeAi' (was 'skipAd')
  ...
  // Claude - class claudeAiHandler fixes: added missing consoleLogId variable
  const consoleLogId        = Math.random().toString(36).substring(2, 8);
  // Claude - class claudeAiHandler fixes: corrected typo 'aiAgen' → 'claudeAi'
  const MODULE_NAME         = 'claudeAi.core';

  // ---------------------------------------------------------------------------
  // Claude - class claudeAiHandler fixes: module-global variable to hold
  // adapter-provided options (apiKey, model, maxTokens, etc.)
  ...
  class claudeAiHandler {
    ...
    // Claude - class claudeAiHandler fixes: constructor now accepts options
    ...
    constructor(options) {
      ...
      if (_adapterOptions.apiKey) {
        consoleLog('INFO', MODULE_NAME, 'claudeAiHandler: API key provided via adapter config');
      } else {
        consoleLog('WARN', MODULE_NAME, 'claudeAiHandler: no API key found in adapter config');
      }

      this.init();

      // Claude - class claudeAiHandler fixes: refresh the AgentUI settings
    ...
    // Claude - class claudeAiHandler fixes: public accessor so external
  ...
  return {
    ...
    // Claude - class claudeAiHandler fixes: expose adapter options so
...
const KnowledgeBase = (() => {
  ...
  async function clearAll() {
    ...
    for (const e of entries) {
      await Storage.remove(PREFIX + e.id);
    }
    entries = [];
    renderList();
  ...
  return {
    loadAll, addUrl, handleDrop, handleFiles, addPaste,
  ...
  async function send() {
    ...
    try {
      ...
      let systemPrompt = settings.systemPrompt ||
        'You are a helpful AI assistant. Answer questions accurately and politely in my language.';
      ...
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: parseInt(settings.maxTokens) || 4096,
          system: systemPrompt,
          messages: history,
        }),
      });
...
const AgentUI = (() => {
  ...
  function openSettings() {
    ...
    if (!modal) return;
    const s = getSettings();
    // Claude - class claudeAiHandler fixes: API key is no longer editable
    ...
    document.getElementById('modelSelect').value = s.model || 'claude-sonnet-4-20250514';
    document.getElementById('systemPromptInput').value = s.systemPrompt || '';
    document.getElementById('maxTokensInput').value = s.maxTokens || 4096;
  ...
  async function saveSettings() {
    // Claude - class claudeAiHandler fixes: only save non-sensitive settings
    // to storage. The apiKey is provided by the adapter from YAML config
    // and must NOT be written to browser storage.
    const settings = {
    ...
    await Storage.set('agent-settings', JSON.stringify(settings));
    // Claude - class claudeAiHandler fixes: invalidate the settings cache
    // so getSettings() rebuilds from storage + adapter options
  ...
  function getSettings() {
    // Claude - class claudeAiHandler fixes: merge adapter-provided options
    // (especially apiKey) with user-editable settings from storage.
    // Adapter options take precedence for sensitive fields.
    if (getSettings._cache) return getSettings._cache;
    // fallback defaults — apiKey comes from adapter config, never from storage
    const adapterOpts = (typeof claudeAi !== 'undefined' && claudeAi.getAdapterOptions)
      ? claudeAi.getAdapterOptions()
      : {};
    return {
      apiKey: adapterOpts.apiKey || '',
      model: adapterOpts.model || 'claude-sonnet-4-20250514',
      systemPrompt: '',
      maxTokens: adapterOpts.maxTokens || 4096
    };
  }

  async function loadSettings() {
    try {
      // Claude - class claudeAiHandler fixes: read adapter options for
      // sensitive data (apiKey) that must not come from storage.
      const adapterOpts = (typeof claudeAi !== 'undefined' && claudeAi.getAdapterOptions)
        ? claudeAi.getAdapterOptions()
        : {};

      const raw = await Storage.get('agent-settings');
      if (raw) {
        const stored = JSON.parse(raw);
        // Claude - class claudeAiHandler fixes: merge stored user settings
        // with adapter-provided options. Adapter values (apiKey, etc.)
        // always override anything that may have been persisted earlier.
        getSettings._cache = Object.assign(
          {},
          { apiKey: '', model: 'claude-sonnet-4-20250514', systemPrompt: '', maxTokens: 4096 },
          stored,
          {
            apiKey:    adapterOpts.apiKey    || '',
            model:     adapterOpts.model     || stored.model     || 'claude-sonnet-4-20250514',
            maxTokens: adapterOpts.maxTokens || stored.maxTokens || 4096
          }
        );
      } else {
        // no stored settings — use adapter defaults
        getSettings._cache = {
          apiKey:       adapterOpts.apiKey    || '',
          model:        adapterOpts.model     || 'claude-sonnet-4-20250514',
          systemPrompt: '',
  ...
  return {
    toggleSidebar, openSettings, closeSettings, saveSettings,
    getSettings, loadSettings, updateStatusBar, clearChat, handleInputKey
  };
})();

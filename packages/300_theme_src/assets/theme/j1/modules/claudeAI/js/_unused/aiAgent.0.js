/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/claudeAi/js/claudeAi.js
...
(function (root, factory) {
  ...
  class claudeAiHandler {
    ...
    /**
     * Build the switch element and wire up the change listener.
     * If a static element with id="playlistModeSwitch" already exists
     * in the DOM, it is reused instead of creating a duplicate.
     * The switch is inserted before the sort <select> element.
     * @private
     */
    init() {
      consoleLog('INFO', MODULE_NAME, 'playlistManager: modeSwitchHandler initialized');
      // (ENVIRONTMENT === 'dev') && logger.info('\n'+ 'playlistModeSwitchHandler: modeSwitchHandler initialized');
    }
...
const AgentUI = (() => {
  ...
  function getSettings() {
    ...
    return { apiKey: '', model: 'claude-sonnet-4-20250514', systemPrompt: '', maxTokens: 4096 };
  }
  ...
  async function loadSettings() {
    try {
      ...
      if (raw) {
        getSettings._cache = JSON.parse(raw);
      }
    } catch { /* use defaults */ }
    updateStatusBar();
  }

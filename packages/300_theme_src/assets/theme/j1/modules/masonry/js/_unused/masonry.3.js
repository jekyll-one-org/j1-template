/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/masonry/js/masonry.js (3)
...
// -------------------------- masonryDefinition --------------------------------
  ...
  proto._resetLayout = function() {
    ...
    // claude - Fix Masonry module error #1
    // Fix: replaced manual for-loop with Array(n).fill(0) for
    // clearer intent and single-allocation initialization
    // reset column Y
    //
    // Guard the array allocation against an invalid column count.
    // On a timing/race condition (container not yet attached, hidden,
    // or not yet measurable), measureColumns() may compute `this.cols`
    // as NaN or Infinity. The upstream for-loop silently tolerated a
    // NaN count (loop body never ran), but `new Array(NaN)` throws
    // "RangeError: Invalid array length". Normalize `this.cols` itself
    // (not just a local copy) so all downstream consumers of `this.cols`
    // stay consistent with the length of `this.colYs`.
    //
  ...
  proto.measureColumns = function() {
    ...
    // claude - Fix Masonry module error #1
    // Sanitize the computed column count. If the container width could
    // not be measured (race condition on init: element detached from the
    // DOM or display:none), `containerWidth` and/or `columnWidth` end up
    // as NaN or 0, which yields cols = NaN (0/0, undefined+n) or
    // Infinity (n/0). Math.max(NaN, 1) returns NaN and would propagate
    // the invalid value into `this.cols`. Fall back to a single column;
    // the next layout()/resize pass recomputes the real value.
    //
    if ( !isFinite( cols ) ) {
      cols = 1;
    }
  ...
  proto.getContainerWidth = function() {
    ...
    // claude - Fix Masonry module error #1
    // getSize() returns `undefined` for a detached or non-measurable
    // container (and `size.innerWidth` can be negative for zero-size,
    // padded/bordered elements). An `undefined` containerWidth poisons
    // all arithmetic in measureColumns() with NaN, which ultimately
    // triggered "RangeError: Invalid array length" in _resetLayout().
    // Coerce to a finite, non-negative number so the NaN can never enter
    // the column calculation.
    //
    var innerWidth = size && size.innerWidth;
    this.containerWidth = ( isFinite( innerWidth ) && innerWidth > 0 ) ? innerWidth : 0;
  };

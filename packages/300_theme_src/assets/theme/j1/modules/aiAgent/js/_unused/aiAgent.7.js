/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/aiAgent/js/aiAgent.js (7)
...
// under the aiAgent namespace — matching the skipad.js architecture.
(function (root, factory) {
  ...
  const KnowledgeBase = (() => {
    ...
    // claude - limit KB size #5: storage key, fallback path, and maxSize are
    // now read from YAML config via AgentUI.getSettings(). Defaults are kept
    // as constants for the case when settings are not yet available.
    const DEFAULT_STORAGE_KEY   = 'ai-agent';
    const DEFAULT_FALLBACK_JSON = '/assets/data/apps/aiAgent/knowledgebase.json';
    const DEFAULT_MAX_SIZE_KB   = 2048;

    let entries = [];

    // claude - limit KB size #5: helper to read KB storage config from YAML
    function getStorageConfig() {
      try {
        ...
        return {
          ...
          maxSize:  s.storageMaxSize  || DEFAULT_MAX_SIZE_KB
        };
      } catch {
        return {
    ...
    // claude - limit KB size #5: compute current storage size in bytes
    function getCurrentSizeBytes() {
      const json = JSON.stringify(entries);
      return new Blob([json]).size;
    }

    // claude - limit KB size #5: check if adding content would exceed maxSize
    function wouldExceedLimit(newContentLength) {
      const cfg          = getStorageConfig();
      const maxBytes     = cfg.maxSize * 1024;
      const currentBytes = getCurrentSizeBytes();
      // estimate: new entry JSON overhead ~200 bytes + content length * 2 (UTF-16 worst case)
      const estimatedAdd = newContentLength * 2 + 200;
      return (currentBytes + estimatedAdd) > maxBytes;
    }

    // claude - limit KB size #5: update the storage usage bar and text in the UI
    function updateStorageUsage() {
      const cfg          = getStorageConfig();
      const maxBytes     = cfg.maxSize * 1024;
      const currentBytes = getCurrentSizeBytes();
      const currentKB    = (currentBytes / 1024).toFixed(1);
      const maxKB        = cfg.maxSize;
      const percentage   = maxBytes > 0 ? Math.min((currentBytes / maxBytes) * 100, 100) : 0;

      const textEl = document.getElementById('kbStorageText');
      const barEl  = document.getElementById('kbStorageBarFill');

      if (textEl) {
        textEl.textContent = `${currentKB} KB / ${maxKB} KB (${percentage.toFixed(0)}%)`;
      }
    ...
    // claude - limit KB size #5: use config-based storage key
    ...
    async function loadAll() {
      entries = [];
      // claude - limit KB size #5: read storage key and fallback from YAML config
      const cfg = getStorageConfig();
    ...
    async function addUrl() {
      ...
      try {
        ...
        // claude - limit KB size #5: check storage limit before adding
    ...
    function renderList() {
      ...
      if (!entries.length) {
        ...
        // claude - limit KB size #5: reset storage usage bar
        updateStorageUsage();
        return;
      }
      ...
      // claude - limit KB size #5: update storage usage bar from YAML config
      updateStorageUsage();

      // claude - accumulate IIFEs #1: onclick handler now references
      // aiAgent.KnowledgeBase.deleteEntry() since KnowledgeBase is no
      // longer a standalone global but lives under the aiAgent namespace.
      container.innerHTML = entries.map(e => `
        <div class="kb-entry" data-id="${e.id}">
          ...
          <button class="kb-entry-delete"
            onclick="aiAgent.KnowledgeBase.deleteEntry('${e.id}')" title="Delete">
      ...
      try {
        ...
        for (const entry of imported) {
          if (entry.id && entry.content) {
            // claude - limit KB size #5: check limit before each imported entry
    ...
    async function clearAll() {
      ...
      // claude - limit KB size #5: remove the configured storage key
      entries = [];
      const cfg = getStorageConfig();
  ...
  const AgentChat = (() => {
    ...
    async function send() {
      ...
      // content sent to the API so the request context is set (e.g. "SkipAd - ").
      ...
      // Add user message (display original text in UI)
      ...
      try {
        ...
        // Build system prompt
        let systemPrompt = settings.systemPrompt || 'Always respond in the same language the user writes in.\nMatch the language of each message.\nThe knowledge base may be in a different language.\nThis rule overrides any language used in the knowledge base context below.\nAlways match the language of the most recent user message'
  ...
  const AgentUI = (() => {
    ...
    function getSettings() {
      ...
      // adapterOptions instead of going through aiAgent.getAdapterOptions().
     ...
     // claude - add userPrompt #1: expose userPrompt from adapter config
      return {
        ...
        systemPrompt:         adapterOpts.systemPrompt          || 'Always respond in the same language the user writes in.\nMatch the language of each message.\nThe knowledge base may be in a different language.\nThis rule overrides any language used in the knowledge base context below.\nAlways match the language of the most recent user message',
        maxInputTokens:       adapterOpts.maxInputTokens        || 512,
        maxOutputTokens:      adapterOpts.maxOutputTokens       || 2048,
        userPrompt:           adapterOpts.userPrompt            || 'SkipAd -',
        chatHistoryLocation:  adapterOpts.chatHistoryLocation   || 'localstorage',
        chatHistoryKey:       adapterOpts.chatHistoryKey        || 'agent-chat-history',
        storageFallback:      adapterOpts.storageFallback       || '/assets/data/apps/aiAgent/knowledgebase.json',
        storageLocation:      adapterOpts.storageLocation       || 'localstorage',
        storageKey:           adapterOpts.storageKey            || 'ai-agent',
        storageMaxSize:       adapterOpts.storageMaxSize        || 2048
      };
    }
    ...
    async function loadSettings() {
      try {
        ...
        getSettings._cache = {
          ...
          maxOutputTokens:      adapterOpts.maxOutputTokens       || 2048,
          userPrompt:           adapterOpts.userPrompt            || 'SkipAd -',
          chatHistoryLocation:  adapterOpts.chatHistory.location  || 'localstorage',
          chatHistoryKey:       adapterOpts.chatHistory.key       || 'agent-chat-history',
          storageFallback:      adapterOpts.storage.fallback      || '/assets/data/apps/aiAgent/knowledgebase.json',
          storageLocation:      adapterOpts.storage.location      || 'localstorage',
          storageKey:           adapterOpts.storage.key           || 'ai-agent',
          storageMaxSize:       adapterOpts.storage.maxSize       || 2048
        };
      } catch { /* use defaults */ }
      updateStatusBar();
    }
    ...
    async function clearChat() {
      ...
      container.innerHTML = `
        <div id="welcomeScreen" class="chat-welcome">
          <span class="mdi mdi-robot-outline"></span>
          <h3>Welcome to the Chat ​​Agent for J1 Template</h3>
          <p>
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    consoleLog('INFO', MODULE_NAME, '[Chat Agent] Initialized — ready for requests.');
  });
  ...
  // through the single aiAgent namespace. HTML onclick handlers that
  // previously referenced global names (e.g. KnowledgeBase.deleteEntry)
  // must now use aiAgent.KnowledgeBase.deleteEntry() instead.
  return {

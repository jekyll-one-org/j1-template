/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/aiAgent/js/aiAgent.js (2)
...
const AgentUI = (() => {
  ...
  function openSettings() {
    ...
    // maxTokens are now supplied by the adapter from YAML config.
    // The modal inputs are populated with the current values and disabled.
    const modelSelect = document.getElementById('modelSelect');
    if (modelSelect) {
      modelSelect.value = s.model || 'claude-sonnet-4-20250514';
  ...
  async function saveSettings() {
    ...
    // model, systemPrompt, maxTokens) are now supplied by the adapter

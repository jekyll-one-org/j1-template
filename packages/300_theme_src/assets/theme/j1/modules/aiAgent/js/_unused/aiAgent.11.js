/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/aiAgent/js/aiAgent.js (11)
...
/* Version 1.0.11 for J1 Template */
  ...
  const AgentChat = (() => {
    ...
    async function send() {
      ...
      try {
        ...
        // claude - downgrade md headers in response #1
        assistantText = assistantText.replace(/^(#{1,5}) /gm, '$1# ');

        // claude - ad id to all md headlines #2
    ...
    function appendMessage(role, content) {
      ...
      if (role === 'assistant') {
        ...
        try {
          ...
          // claude - ad id to all md headlines #2
          // convert extended-markdown {#custom_id} tokens inside rendered
          // headings into proper HTML id attributes on the <h> tags;
          // the character class [\w-]+ matches IDs from generateHeaderId()
          // (lowercase alphanumeric) as well as any legacy hyphenated IDs.
          html = html.replace(/<h([1-6])>([\s\S]*?)\s*\{#([\w-]+)\}\s*<\/h\1>/gi,
            '<h$1 id="$3">$2</h$1>');
    ...
    function getSettings() {
     ...
     // claude - add userPrompt #1: expose userPrompt from adapter config
      ...
      return {
        ...
        storageFallback:      stg.fallback                      || '/assets/data/apps/aiAgent/knowledgebase.json',
    ...
    async function clearChat() {
      ...
      container.innerHTML = `
        <div id="welcomeScreen" class="chat-welcome">
          <span class="mdi mdi-robot-outline"></span>
          <h3>Welcome to the Chat ​​Agent</h3>
          <p>

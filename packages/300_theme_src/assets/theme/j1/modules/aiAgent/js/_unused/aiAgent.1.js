/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/aiAgent/js/aiAgent.js (1)
  ...
  class aiAgentHandler {
    ...
    constructor(options) {
      ...
      if (_adapterOptions.apiKey) {
        consoleLog('INFO', MODULE_NAME, 'aiAgentHandler: API key provided via adapter config');
      } else {
        consoleLog('WARN', MODULE_NAME, 'aiAgentHandler: no API key found in adapter config');
      }
...
const AgentUI = (() => {
  ...
  async function saveSettings() {
    // Claude - class aiAgentHandler fixes: only save non-sensitive settings
    ...
    // Claude - class aiAgentHandler fixes: invalidate the settings cache
  ...
  function getSettings() {
    // Claude - class aiAgentHandler fixes: merge adapter-provided options
  ...
  async function loadSettings() {
    try {
      // Claude - class aiAgentHandler fixes: read adapter options for
      ...
      if (raw) {
        ...
        // Claude - class aiAgentHandler fixes: merge stored user settings

...
(function (root, factory) {
  ...
  function publishPluginOptions(plugin) {
    ...
    //
    // The names are taken from the live adapter|module namespaces now.
    //
    // Original (deprecated, preserved for reference):
  ...
  function _toggleHandler(elementId, onClass, apply, label) {
    return function (event) {
      var state = (document.getElementById(elementId).className.includes(onClass)) ? true : false;
      isDev && logger.debug('\n' + `Set ${label} state to: ${state}`);
      apply(state);
    };
  } // END _toggleHandler
  ...
  function _wireCompactPlayer(cfg) {
    ...
    if (compactPlayerShuffleButton) {
      _bindOnce(compactPlayerShuffleButton, 'click',
        _toggleHandler('compact_player_shuffle', 'amplitude-shuffle-on', function (s) { Amplitude.setShuffle(s); }, 'shuffle'),
    ...
    if (compactPlayerRepeatButton) {
      _bindOnce(compactPlayerRepeatButton, 'click',
        _toggleHandler('compact_player_repeat', 'amplitude-repeat-on', function (s) { Amplitude.setRepeat(s); }, 'repeat'),
  ...
  function _wireLargePlayer(cfg) {
    ...
    if (largePlayerShuffleButton) {
      _bindOnce(largePlayerShuffleButton, 'click',
        _toggleHandler('large_player_shuffle', 'amplitude-shuffle-on', function (s) { Amplitude.setShuffle(s); }, 'shuffle'),
    ...
    if (largePlayerRepeatButton) {
      _bindOnce(largePlayerRepeatButton, 'click',
        _toggleHandler('large_player_repeat', 'amplitude-repeat-on', function (s) { Amplitude.setRepeat(s); }, 'repeat'),
    ...
    if (playerHoverPageScrollDisabled) {
      ...
      var largePlayerRight = document.getElementById('large_player_right');
  ...
  // ===========================================================================
  // Claude - J1 amplitudePlayer optimizations #1
  // PRESERVED REFERENCE MATERIAL (additive-only rule)
  //
  // The adapter carried several code regions that were ALREADY DISABLED:
  //
  //   * blocks marked "PREPARED event listener for LATER use", wrapped in
  //     Liquid {% comment %} ... {% endcomment %}
  //   * the fully commented-out method monitorPlayerActiveElementChanges()
  //
  // They are NOT part of the optimized runtime, but nothing is deleted: the
  // regions are preserved here VERBATIM (every line prefixed with "// " so
  // the Liquid tags they contain stay inert in this non-Liquid module file).
  // ===========================================================================

  // ---- adapter lines 1459-1482 ----------------------------------------

...
(function($) {
  ...
  var LunrSearch = (function() {
    ...
    LunrSearch.prototype.displayResults = function(docs) {
      ...
      // Cleanup tags and categories. Remove unwanted categories
      // defined by category_blacklist and trailing (dot) delimiters.
      // Because search results are generated by MustacheJS (having no
      // logic), some cleanups are needed after results generated.
      //
      var j1_config_url = '/assets/data/j1_config.json';
      $.ajax ({
        url: j1_config_url,
        dataType: 'json',
        success: function (data) {
          var category_blacklist  = data.posts.category_blacklist;
          var filter              = category_blacklist.join('\\s+·\\s+|');
          var regex               = '(' + filter + ')';
          ...
          if (docs.length > 0) {
            ...
            // Cleanup black-listed categories
            $('.sr-categories').each(function() {
              sr_categories = $(this).html().replace(re, '');
              $(this).text(sr_categories);
            });

            // Cleanup tags and categories for trailing (dot) delimiters
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          logger.error('\n' + 'failed to retrieve JSON data from: ' + j1_config_url);
        }
      });
    };

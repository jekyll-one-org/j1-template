      ...
      {% for item in attic_options.attics %}
        {% if item.attic.enabled %}
          ...
          function {{attic_id}} (atticOptions) {
            ...
            -------------------------------------------------------------------- {% endcomment %}
           $(window).on('backstretch.before', function (e, instance, index, atticOptions) {
              var evt           = e;
              var inst          = instance;
              var idx           = index;
              var atticOptions  = _this.atticOptions;
...
*/
              var textOverlayHTML = ''
                ...
                + '  <h2 id="head-title-text" class="notoc text-emphasis-stronger">Roundtrip</h2>'
                + '</div>'
                + '<div id="head-tagline" class="head-tagline animate__animated ">'
                + '  <h3 id="head-tagline-tagline" class="notoc">advanced bootstrap modals</h3>'
                + '</div>';
              $('.textOverlay').html(textOverlayHTML);
//            $('.textOverlay').hide();

  ...
  function executeParticles(containerID, json_data) {
    ...
    {% for item in particles_settings.particles %}
      {% if item.particle.enabled %}
        ...
        // particlesJS(containerID, strJSON);
        ...
        if ( j1.getState() === 'finished' ) {
          ...
          var dependencies_met_attic_ready = setInterval (function (options) {
            if ($particles.length != 0) {
              ...
              $.get(dataUrl)
              .done(function (data) {
                  doc = yaml.loadAll(data, 'utf8'), function (doc) {
                  console.log(doc);
                }

                // [0]["id-1"][0].particles
                var objParticles = doc[0]["id-1"][0].particles;
                var strJSON = JSON.stringify(objParticles, null, 2);
                ...
                var strParticles =  '"particles": '
                    strParticles += strJSON;
                    strParticles += ''

                // particlesJSON = strJSON.replace(/[\[\]]/g, '');

...
//            logger.info('\n' + 'initializing module finished');
    ...
    loadGridHTML: function (options, grid) {
      ...
      console.debug('number of Masonry grids found: ' + numGrids);
      ...
      Object.keys(grid).forEach(function(key) {
        if (grid[key].enabled) {
          ...
          console.debug('load HTML data on Slick grid id: ' + grid[key].id);
          j1.loadHTML({
          ...
          console.debug('Slick grid found disabled on id: ' + grid[key].id);
          active_grids--;
        }
      });
      console.debug('Slick sliders loaded in page enabled|all: ' + active_grids + '|' + numGrids);

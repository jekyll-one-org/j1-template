    ...
    def process parent, target, attributes
      ...
      html = %(
        ...
        <script>
          ...
          var dependencies_met_player_exist = setInterval (function (options) {
            var vjsPlayerExist = (document.getElementById("#{video_id}").length) ? true : false;

            if (vjsPlayerExist) {
              var bla = 'puffi';

              clearInterval(dependencies_met_player_exist);
            } // END if 'vjsPlayerExist'

          }, 10); // END 'dependencies_met_player_exist'

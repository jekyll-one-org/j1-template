...
//  var chunksEnd = false;
    ...
    function scanPage(headings) {
      ...
      function scanSection(counter){
        if (counter < headings.length -1) {
          setTimeout(function() {
            counter++;
            yourElement = headings[counter];
//          y = yourElement.getBoundingClientRect().top + window.pageYOffset;
...
//        var chunksEnd = false;
          ...
          if (headingsArray !== null && headingsArray.length > 0) {
            scanPage(headingsArray);
          } else {
            scanFinished = true;
          }

/*
 # -----------------------------------------------------------------------------
 # ~/assets/theme/j1/modules/speak2me/js/speak2me.3.js
...
//  'en-US':  'Microsoft Zira Desktop - English (United States)',
  ...
  function highlightWord(charIndex) {
      ...
      // remove all exising highlights 
      spans.forEach(span => span.classList.remove('highlight'));

      // searh for current word to highlight
      for (var span of spans) {
          ...
          if (charIndex >= currentPos && charIndex < currentPos + wordLength) {
              ...
              // scrool smooth to current word spoken
  ...
  // split spoken text in spans (for highlightning)
    ...
    speak: function(options) {
        ...
        var processSpeech = setInterval(function () {
          if (scanFinished) {
            ...
            // speech synthesis utterance boundary events
            // NOTE: avalable for OS based voices only
...
//                    console.warn('speak2me, text: ' + node.innerText + ', offsetTop not caclulated.');
      ...
      function processTextChunks(speaker, chunks) {
        ...
        // Claude: In the processTextChunks function, modify the
        ...
        // manage highlighting on currently spoken paragraph
        // Re-find the paragraph to ensure we have the current DOM reference
        if (speaker.text && speaker.text.length > 0) {
            var sectionText = textSlice(speaker.text, textSliceLength, minWords);
            if (sectionText !== undefined) {
            // Escape special characters in the selector
            var escapedText = sectionText.replace(/['"]/g, '\\$&');
            var $currentParagraph = $('#content').find("p:contains('" + escapedText + "')").first();
            
            if ($currentParagraph.length > 0) {
                $currentParagraph.addClass('speak-highlighted');
                speaker.$currentHighlight = $currentParagraph; // Store reference
            }
            }
        }
        });
        ...
        // Claude: Modify the 'end' event listener:

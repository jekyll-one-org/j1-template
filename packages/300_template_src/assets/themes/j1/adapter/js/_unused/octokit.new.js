...
j1.adapter['octokit'] = (function (j1, window) {
  ...
  return {
    ...
    init: function () {
      ...
      ep_util_serv_git_pull     = 'http://localhost:' + moduleOptions.utility_server.port + '/git?request=pull';
      ep_util_serv_npm_rebuild  = 'http://localhost:' + moduleOptions.utility_server.port + '/npm?request=rebuild';

      if (j1.checkUserAgent('IE') || j1.checkUserAgent('Edge')) {
        moduleOptions.webhooks.enabled = false;
        logger.warn('browser does not support server side events (SSE)');
        logger.warn('webhooks disabled');
      }

      if (moduleOptions.webhooks.enabled) {
    ...
    messageHandler: function (sender, message) {
      ...
      if (message.type === 'command' && message.action === 'built') {
        ...
        $.when (j1.xhrApi(ep_util_serv_npm_rebuild))
        .then (function(response) {
          // json_message = JSON.stringify(response, undefined, 2);
          json_message = JSON.stringify(response);

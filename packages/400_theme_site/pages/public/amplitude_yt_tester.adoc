...
//
:ytp_api_description:                   false
:ytp_example:                           false
:ytp_audio_player:                      true
:ms_slider_video_example:               false
...
// -----------------------------------------------------------------------------
ifeval::[{ytp_api_description} == true]
[role="mt-4"]
== YouTube Audio
...
ifeval::[{ytp_audio_player} == true]
[role="mt-5"]
== Amplitude YT Player

The example below demonstrates playing Audio tracks from YouTube Videos using
an AmplitudJS (large) player. The J1 template uses a *plugin* (ytp) to extend
the native functionality of the AmplitudeJS API.

[role="mt-4 mb-5"]
[CAUTION]
====
Playing audio tracks is currently supported only for videos from YouTube (YT).
Note that the YT plugin embeds a YP player and mimics the AmplitudeJS player
buttons and can only be used with the J1 Template.
====
...
ifeval::[{ms_slider_video_example} == true]
[[ms_video]]

...
description: >
                                        ...
                                        functionality based on powerful open source libraries.
keywords: >
...
open source libraries like Lightbox2, Master Slider, LightGallery, or
...
J1 Carousel is based on _OWL Carousel V1_ in the latest version. OWL Carousel
is a clean and neat _jQuery_ slider plugin for creating fully responsive
and touch-enabled carousel sliders.
...
A base image show is useful, for example, as an animated (or not animated)
...
a lightbox to enlarge images full size. For the example below, the default
lightbox for Jekyll One is used.

.Image Carousel + Lightbox
carousel::demo_cats[role="mb-3"]

The J1 lightbox module offers a bunch of impressive features for displaying
images. For example, the module can display all images (of a carousel) as
a group. If one picture is opened, others are browsed by easy-to-use control
buttons.
...
WARNING: The support of using lightboxes is only available for the MS Lite
version of J1 Theme. The product versions *MS Lite* and *MS Pro* does not
support lightboxes on sliders out-of-the-box.

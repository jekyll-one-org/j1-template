...
description: >
                                        Amplitude is a JavaScript library to control the design of audio mediaelements on web pages. The concept allows designers to fully on the look and feel of audio players without deep scripting knowledge.
keywords: >
...
However, the browser used fully controls the audio playback interface, which
may limit or even break a page's design if audio media is used.

Amplitude for the J1 Template offers a straightforward solution for web
...
Amplitude comes with a variety of HTML elements. Each element is picked up
by CSS classes prefixed with *amplitude-* and depending on the player type,
an attribute that relates to the index of a song in your list of songs or a
...
Amplitude has a standardized way to reference an element whether it's a
...
There are a variety of public functions that Amplitude exposes to the
...
The examples demonstrates playing *Audio* track from *YouTube Video* using an
Amplitude player. The J1 Template uses the video *plugin ytp* to extend the
native functionality of the Amplitude API to playback YouTube files *only* for
the *audio part*.

[role="mt-4 mb-5"]
[NOTE]
====
Amplitude YT Player can only be used with the J1 Template using the Amplitude
plugin *ytp*. Currently, only players of type *large* are supported.

The YT plugin embeds a YouTube (iframe) player and mimics all Amplitude
player buttons and controls to manage a *playlist* of *videos* the same as
known for native audio files.
====

---
title:                                  Vimeo iFrame API
title_extention:                        Manage VM Players
tagline:                                Manage VM Players

date:                                   2020-11-04
last_modified:                          2025-06-17

description: >
                                        Vimeo offered an API (Application Programming Interface)
                                        called the Vimeo Player API, which allowed developers to
                                        interact with and control Vimeo videos embedded on websites
                                        or applications.
keywords: >
                                        open source, free, template, jekyll, jekyllone, web,
                                        sites, static, jamstack, bootstrap,
                                        html, html5, video, support, vimeo

categories:                             [ Manual ]
tags:                                   [ API, Vimeo ]

image:
  path:                                 /assets/image/module/attic/1920x1280/j1-videojs-api.jpg
  width:                                1920
  height:                               1280
...
permalink:                              /pages/public/manuals/integrations/videojs/vimeo_api/
...
resource_options:
  ...
  - attic:
      slides:
        - url:                          /assets/image/module/attic/1920x1280/j1-videojs-api.jpg
          alt:                          J1 Vimeo iFrame API Poster
---
...
dynamically via the https://www.requirejs.org/docs/api.html#jsfiles[RequireJS, {browser-window--new}]
load system.
...
link:#embed-options[embed options, {browser-window--new}]).

*NOTE:* If the video
https://vimeo.zendesk.com/hc/en-us/articles/224817847-Privacy-settings-overview[privacy settings, {browser-window--new}]
...
If the video
https://vimeo.zendesk.com/hc/en-us/articles/224817847-Privacy-settings-overview[privacy settings, {browser-window--new}]
are *Private*, instead of providing a `data-vimeo-id` attribute, you will
need to provide the full video URL in a data-vimeo-url` attribute and
...
https://github.com/vimeo/player.js/blob/master/docs/migrate-from-froogaloop.md[migration doc, browser-window--new]
for details on how to update your code to use this library.

[role="mt-4"]
== Using with a module bundler

If you’re using a module bundler like https://webpack.js.org[webpack, browser-window--new]
or https://rollupjs.org/[rollup, browser-window--new], the exported object
will be the Player constructor (unlike the browser where it is attached to
`window.Vimeo`):
...
Similarly, if you’re using https://www.requirejs.org[RequireJS, browser-window--new]
in the browser, it will also import the Player constructor directly:
...
<script>
  require(['https://player.vimeo.com/api/player.js'], (Player) => {
    const iframe = document.querySelector('iframe');
...
an object passed to the `Vimeo.Player` constructor. A complete list of
embed options can be found in Vimeo's official
https://developer.vimeo.com/player/sdk/embed[SDK documentation, {browser-window--new}].

[cols="3,2a,7a", subs=+macros, options="header", width="100%", role="rtable mt-5"]
|===
|Option |Default |Description

|`id` _or_ `url`
|*NO* defaults
|The ID or the URL of the video on Vimeo. You
must supply one of these values to identify the video.

[NOTE]
====
When the video's
https://vimeo.com/help/sso?redirect_to=https://help.vimeo.com/hc/en-us/articles/12426199699985-Overview-of-video-privacy-settings[privacy setting, {browser-window--new}]
is `Unlisted`, you must use the URL, and the URL must include the `h`
parameter. For more information, see our
https://developer.vimeo.com/player/sdk/basics#connecting-to-a-vimeo-player[introductory guide, {browser-window--new}].
====
...
AirPlay is a wireless streaming technology developed by Apple. In the context
of video players like Vimeo, AirPlay wouldn't be directly built into Vimeo
itself (as Vimeo isn't an Apple product). +

[TIP]
====
However, if you're using Vimeo on an Apple device (iPhone, iPad, Mac) and
you want to watch the video on a larger screen, you might be able to use
AirPlay to stream the video from your device to an AirPlay-compatible
devices.
====
...
This option has no effect if your video doesn't have additional audio
tracks uploaded to it.
...
|`background`
|`false`
|Whether the player is in background mode, which hides the playback
controls, enables autoplay, and loops the video.
...
This option has no effect if your video doesn't have captions or
subtitles.
...
|`color`
|*NO* defaults
|The hexadecimal accent color value of the playback controls, which
is normally `00ADEF`. The embed settings of the video might override
this value. For more information about player colors, refer to the
`colors` option.

|`colors`
|*NO* defaults
|The hexadecimal color values of the player. The embed settings of the
video might override these values. The order of the player colors is
`[Primary, Accent, Text/Icon, Background]`, with corresponding default
values of `["000000", "00ADEF", "FFFFFF", "000000"]`.

|`controls`
|`true`
|Whether to display the player's interactive elements, including the
play bar and sharing buttons.

Set this option to `false` for a chromeless experience. To control playback
when the play/pause button is *hidden*, set `autoplay` to `true`, use keyboard
controls (which remain active), or implement our player SDK.

|`dnt`
|`false`
|Whether to prevent the player from tracking session data, including
cookies.

[NOTE]
====
Keep in mind that setting this argument to `true` also blocks
https://developer.vimeo.com/player/sdk/basics#connecting-to-a-vimeo-player[video stats, {browser-window--new}].
====

|`end_time`
|*NO* defaults
|In Segmented Playback mode, the time in seconds where playback ends
for the video. Setting this field enables Segmented Playback mode,
where the video is presented as a segment of the original as defined
by this field and the `start_time` field, with a duration in
...
|`interactive_markers`
|`true`
|Whether to display markers representing the timestamp where hotspots
appear on an interactive video.

|`interactive_params`
|*NO* defaults
|Key-value pairs representing dynamic parameters that are utilized on
interactive videos with dynamic elements, such as `title=my-video,subtitle=interactive`.

|`keyboard`
|`true`
|Whether to enable keyboard input to trigger player events. This setting
doesn't affect tab control.
...
|`maxheight`
|*NO* defaults
|The height of the video in pixels, where the video won't exceed its
native height, no matter the value of this field.

|`maxwidth`
|*NO* defaults
|The width of the video in pixels, where the video won't exceed its
native width, no matter the value of this field.

|`muted`
|`false`
|Whether the video is muted upon loading. The `true` value is required
for the autoplay behavior in some browsers.

|`pip`
|`true`
|Whether to include the picture-in-picture button among the player
controls and enable the Picture-in-Picture API, where supported.

|`play_button_position`
|`auto`
|The position of the play button within the embeddable player.
Other possible values include `bottom` and `center`.

|`playsinline`
|`true`
|Whether the video plays inline on supported mobile devices.

To force the device to play the video in fullscreen mode instead,
...
Use `auto` for the best possible quality given available bandwidth and
other factors. You can also specify `360p`, `540p`, `720p`, `1080p`, `2k`,
and `4k`.
...
|`responsive`
|`false`
|Whether to return a _responsive_ *embed code*, or one that provides
intelligent adjustments based on viewing conditions.
...
|`start_time`
|`0`
|In Segmented Playback mode, the time in seconds where playback starts
for the video; see the `end_time` field for more information.

|`texttrack`
|*NO* defaults
|The text track to display with the video.

Specify the text track by its language code (`en`), the language code
and locale (`en-US`), or the language code and kind (`en.captions`).

For this argument to work, the video must already have a text track of
the given type. See the
https://help.vimeo.com/hc/en-us/articles/12426156795409-About-captions-and-subtitles[Help Center, {browser-window--new}]
or https://developer.vimeo.com/api/upload/texttracks[Working with Text Track Uploads, {browser-window--new}]
...
|`watch_full_video`
|`true`
|Whether to show the Watch Full Video button when Segmented Playback
mode is enabled.
...
If the video
https://vimeo.zendesk.com/hc/en-us/articles/224817847-Privacy-settings-overview[privacy settings, {browser-window--new}]
are *Private*, instead of providing an `id` argument, you will need to
provide the full video URL as a `url` argument and include
the `h` parameter.
====
...
values in the
https://developer.mozilla.org/en-US/docs/Web/API/RemotePlayback/state[Remote Playback API, {browser-window--new}].
...
Get the https://vimeo.com[vimeo.com, {browser-window--new}] url for the video.
...
See https://webtiming.github.io/timingobject/[timingobject, {browser-window--new}]
for reference.
...
See this https://salmoro.github.io/vimeo-timing-object-demo/dist[demo app, {browser-window--new}]
...
https://developer.mozilla.org/en-US/docs/Web/API/RemotePlayback/watchAvailability[RemotePlayback.watchAvailability() API, {browser-window--new}],

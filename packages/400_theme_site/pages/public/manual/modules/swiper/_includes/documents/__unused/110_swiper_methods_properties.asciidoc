...
After we initialise the slider, we get an *instance* of `Swiper` (stored
in a variable like `swiper` in the example above) that exposes helpful
*methods* and *properties* you can call from your code. If you no longer
have the variable, you can fetch it again from the DOM:

[source, js]
----
const mySwiper = document.querySelector('.my-swiper-container').swiper;
mySwiper.slideNext();
----

The follwing sections list all *properties* first and then all *methods*
...
of the swiper container

|link:#method-swiper-updateSlides[swiper.updateSlides()] | |Recalculate
the number of slides and their offsets. Useful after you add/remove
...
Please note that the `this` keyword inside an event handler always points
to the Swiper instance.

[width="100%",cols="34%,33%,33%",options="header",]
|===
|Name |Arguments |Description
|link:#event-activeIndexChange[activeIndexChange] |(swiper) |Event will
be fired on active index change

|link:#event-afterInit[afterInit] |(swiper) |Event will be fired right
after initialization

|link:#event-beforeDestroy[beforeDestroy] |(swiper) |Event will be fired
right before Swiper destroyed

|link:#event-beforeInit[beforeInit] |(swiper) |Event will be fired right
before initialization

|link:#event-beforeLoopFix[beforeLoopFix] |(swiper) |Event will be fired
right before "loop fix"

|link:#event-beforeResize[beforeResize] |(swiper) |Event will be fired
...
speed, internal) |Event will be fired before transition start
...
the user clicks or taps Swiper. Receives `pointerup` event as an argument.
...
fired when the user double-clicks or double-taps Swiper

|link:#event-doubleTap[doubleTap] |(swiper, event) |Event will be fired
when the user double-taps Swiper's container. Receives `pointerup` event
as an argument
...
when Swiper progress is changed, as an argument it receives progress
that is always from 0 to 1

|link:#event-reachBeginning[reachBeginning] |(swiper) |Event will be
fired when Swiper reaches its beginning (initial position)

|link:#event-reachEnd[reachEnd] |(swiper) |Event will be fired when
Swiper reaches the last slide
...
will be fired every time swiper starts animation. Receives current
transition duration (in ms) as an argument

|link:#event-setTranslate[setTranslate] |(swiper, translate) |Event will
be fired when swiper's wrapper changes its position. Receives current
translate value as an argument
...
fired when the user touches and moves a finger over Swiper and moves it. Receives
...
click/tap on Swiper. Receives `pointerup` event as an argument.
...
when the user releases Swiper. Receives `pointerup` event as an argument.

|link:#event-touchMove[touchMove] |(swiper, event) |Event will be fired
when the user touches and moves a finger over Swiper. Receives `pointermove`
...
fired when the user touches Swiper. Receives `pointerdown` event as an
argument.
...
fired at the beginning of a transition.

|link:#event-unlock[unlock] |(swiper) |Event will be fired when swiper
is unlocked (when `watchOverflow` is enabled)

|link:#event-update[update] |(swiper) |Event will be fired after a
`swiper.update()` call
|===

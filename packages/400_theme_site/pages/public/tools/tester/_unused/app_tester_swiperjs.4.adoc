...
====
Have a look a
link://daneden.github.io/animate.css/[Dan Eden's home page, {browser-window--new}]
to see all *possible* styles available to be created based on pure CSS. Some of
them are implemented for J1 Swipper Apps for animation. For more details, see
...
Layouts in the J1 Template for SwiperJS are *transformations* of a Swiper to
rebuild it for  a more complex visual *presentation* for the Swiper's slides.
The Layout of a swiper  can *improve* the *useability* or make the presentation
...
++++
<div class="swiper-title"> <i class="mdib mdib-view-carousel mdib-24px mr-2"></i> Layout Thumb · Right + Captions </div>
...
.Extended Slider · Layout Neighbor
...
.Extended Slider · Layout Panorama
...
++++
<div class="swiper-title mt-4"> <i class="mdib mdib-view-carousel mdib-24px mr-2"></i> Extended Slider · Layout Expanding </div>
<div id="app">
  ...
  function createLayoutExpanding(el) {
    ...
    const initEvents = (swiper) => {
      swiper.slides.forEach((slideEl) => {
        const containerEl = slideEl.querySelector('.expanding-slide-container');
        const coverEl     = slideEl.querySelector('.expanding-slide-cover');
        const contentEl   = slideEl.querySelector('.expanding-slide-content');
    ...
    const swiper = new Swiper(swiperEl, {
      ...
      on: {
        init(swiper) {
          swiper.slides.forEach((slideEl) => {
            const coverImageEl = slideEl.querySelector('.expanding-slide-cover img');
        ...
        slideChange(swiper) {
          const openedContainerEl = swiper.wrapperEl.querySelector('.expanding-slide-opened');

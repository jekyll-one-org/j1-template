...
$(function() {
  ...
  // Initialize Swiper Neighbor Effect
  function createNeighborEffect(swiper) {
...
<div id="app">
  ...
  <div id="expanding_card" class="expanding-card mt-4 mb-5">
    <div class="swiper">
      <div class="swiper-wrapper">
        ...
        <div class="swiper-slide">
          ...
          <div class="expanding-card-container mb-4">
            <!-- Expanding collection content that opens underneath the cover image on click -->
            <div class="expanding-card-content">
              <div class="expanding-card-content-inner">
                <!-- Put any required content here -->
                <div class="card-content">
                  <div class="card-content-title">Jameson Street, CA90030</div>
                  <div class="card-content-avatars">
                    <img src="/assets/image/module/swiper/expanding_swiper/avatar1.jpg">
                    <!--img src="/assets/image/module/swiper/expanding_swiper/avatar2.jpg" -->
                    <!-- img src="/assets/image/module/swiper/expanding_swiper/avatar3.jpg" -->
                    <!-- img src="/assets/image/module/swiper/expanding_swiper/avatar4.jpg" -->
                  </div>
                  <div class="card-content-rating">
                    <img src="/assets/image/module/swiper/expanding_swiper/scalable/star-orange.svg" />
                    <img src="/assets/image/module/swiper/expanding_swiper/scalable/star-orange.svg" />
                    <img src="/assets/image/module/swiper/expanding_swiper/scalable/star-orange.svg" />
                    <img src="/assets/image/module/swiper/expanding_swiper/scalable/star-orange.svg" />
                    <img src="/assets/image/module/swiper/expanding_swiper/scalable/star-gray.svg" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="expanding-card-cover">
              <img src="/assets/image/module/swiper/expanding_swiper/tokyo.jpg">
              <div class="card-cover-title">Tokyo</div>
              <!-- div class="card-cover-coords">
        ...
        <div class="swiper-slide">
          <div class="expanding-card-container">
            ...
            <div class="expanding-card-cover">
              <img src="/assets/image/module/swiper/expanding_swiper/san-francisco.jpg">
              <div class="card-cover-title">San Francisco</div>
              <!-- div class="card-cover-coords">
                <span>North LAT 36</span>
                <span>East LON 37</span>
              </div -->
        ...
        <div class="swiper-slide">
          <div class="expanding-card-container">
            ...
            <div class="expanding-card-cover">
              <img src="/assets/image/module/swiper/expanding_swiper/london.jpg">
              <div class="card-cover-title">London</div>
              <!-- div class="card-cover-coords">
                <span>North LAT 36</span>
                <span>East LON 37</span>
              </div -->
        ...
        <div class="swiper-slide">
          <div class="expanding-card-container">
            ...
            <div class="expanding-card-cover">
              <img src="/assets/image/module/swiper/expanding_swiper/moscow.jpg">
              <div class="card-cover-title">Moscow</div>
              <!-- div class="card-cover-coords">
                <span>North LAT 36</span>
                <span>East LON 37</span>
              </div -->
...
$(function() {
  ...
  // Initialize Expanding Card Effect
  function createExpandingCardEffect(el) {
    const swiperEl = el.querySelector('.swiper');

    const calcOffsets = (slideEl) => {
      const coverEl = slideEl.querySelector('.expanding-card-cover');
      const contentEl = slideEl.querySelector('.expanding-card-content');
      if (!contentEl || !coverEl) return;
      const { offsetWidth: coverWidth, offsetHeight: coverHeight } = coverEl;
      slideEl.style.setProperty(
        '--expanding-card-cover-height',
        `${coverHeight}px`,
      );
      const { offsetHeight: contentHeight, offsetWidth: contentWidth } =
        contentEl;
      const props = {
        '--expanding-card-scale-x': (coverWidth / contentWidth) * 0.95,
        '--expanding-card-scale-y': (coverHeight / contentHeight) * 0.95,
      };
      ...
      swiper.slides.forEach((slideEl) => {
        ...
        coverEl.expandingCollectionClickHandler = () => {
          if (!contentEl || !slideEl.classList.contains('swiper-slide-active'))
            return;
          containerEl.classList.toggle('expanding-card-opened');
        };
      ...
      on: {
        init(s) {
          ...
          requestAnimationFrame(() => {
            el.classList.add('expanding-card-initialized');
          });
          initEvents(s);
        },
        slideChange(s) {
          const openedContainerEl = s.wrapperEl.querySelector(
            '.expanding-card-opened',
          );
          if (openedContainerEl) {
            openedContainerEl.classList.remove('expanding-card-opened');
          }
        },
        imagesReady(s) {
          el.classList.remove('expanding-card-initialized');
          s.slides.forEach((slideEl) => {
            calcOffsets(slideEl);
          });
          el.classList.add('expanding-card-initialized');
  ...
  var dependencies_met_page_ready = setInterval (() => {
    ...
    if (atticFinished) {
      const sliderEl = document.querySelector('#expanding_card');

      createExpandingCardEffect(sliderEl);
...
.expanding-card .card-cover-title {
...
.expanding-card .card-cover-coords {
  padding: 8px 16px;
  width: 100%;
  box-sizing: border-box;
  position: absolute;
...
.expanding-card .card-content-title {
  text-align: center;
  font-size: 16px;
  margin-bottom: 8px;
  font-weight: 600;
...
.expanding-card .card-content-avatars {
  display: flex;
}

.expanding-card .card-content-avatars img {
  border-radius: 50%;
  width: 32px;
  height: 32px;
...
.expanding-card .card-content-avatars img + img {
  margin-left: -16px;
}

.expanding-card .card-content-rating {
  display: flex;
}

.expanding-card .card-content-rating img {
  width: 16px;
  height: 16px;
}
...
:root {
  --expanding-card-content-bg-color: #fff;
  --expanding-card-content-vertical-offset: 32px;
  --expanding-card-content-horizontal-offset: 16px;
  --expanding-card-content-padding: 16px;
  --expanding-card-content-border-radius: 0px;
  --expanding-card-cover-border-radius: 0px;
  --expanding-card-inactive-cover-opacity: 0.5;
  --expanding-card-opened-translate-y: -64px;
  --expanding-card-cover-box-shadow: 0px 8px 16px rgba(0, 0, 0, 0.7);
  --expanding-card-opened-cover-box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.5);
}
...
.expanding-card-initialized .expanding-card-content,
.expanding-card-initialized .expanding-card-cover,
.expanding-card-initialized .expanding-card-cover::before {
  transition-duration: 300ms;
}

.expanding-card-container, .expanding-card-cover {
...
.expanding-card-cover {
  opacity: var(--expanding-card-inactive-cover-opacity);
  position: relative;
}

.expanding-card-cover img {
  width: 100%;
  border-radius: var(--expanding-card-cover-border-radius);
  display: block;
  margin: 0;
  position: relative;
  z-index: 2;
...
.expanding-card-cover::before {
  ...
  box-shadow: var(--expanding-card-cover-box-shadow);
}

.swiper-slide.swiper-slide-active .expanding-card-cover {
  opacity: 1;
  cursor: pointer;
}

.swiper-slide:not(.swiper-slide-active) .expanding-card-cover::before {
  box-shadow: none;
}

.swiper-slide:not(.swiper-slide-active) .expanding-card-content {
  opacity: 0;
}
...
.expanding-card-content {
  position: absolute;
  box-sizing: border-box;
  left: calc(-1 * var(--expanding-card-content-horizontal-offset));
  right: calc(-1 * var(--expanding-card-content-horizontal-offset));
  top: 0;
  z-index: 0;
  padding-top: calc(var(--expanding-card-cover-height) - var(--expanding-card-content-vertical-offset));
  border-radius: var(--expanding-card-content-border-radius);
  transform: scaleX(var(--expanding-card-scale-x)) scaleY(var(--expanding-card-scale-y));
  transform-origin: center top;
  background: var(--expanding-card-content-bg-color);
}

.expanding-card-content-inner {
  padding: var(--expanding-card-content-padding);
  box-sizing: border-box;
}

.expanding-card-opened .expanding-card-cover {
  transform: translateY(var(--expanding-card-opened-translate-y));
}

.expanding-card-opened .expanding-card-cover::before {
  box-shadow: var(--expanding-card-opened-cover-box-shadow);
}

.expanding-card-opened .expanding-card-content {
  transform: scale(1) translateY(calc(var(--expanding-card-opened-translate-y) + var(--expanding-card-content-vertical-offset)));
  opacity: 1;

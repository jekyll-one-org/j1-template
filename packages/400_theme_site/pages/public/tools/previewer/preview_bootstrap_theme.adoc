...
Themes for the Bootstrap framework are used to customize the appearance of
a website based on the basic rules and definitions of Bootstrap's (CSS) styles.
The use or change of a theme does not alter the functionality of the underlying
...
theming classes and background-color utilities. Choose the light style
`.navbar-light` for use with light background colors, or the daek style
'.navbar-dark' for dark background colors. Then, customize with the
background `.bg-*` utilities.

++++
<div class="title fw-bold mb-2">Output example</div>
...
The HTML blockquote element `<blockquote>` specifies a section with content
quoted from another source. Blockquotes are generally displayed with indented
...
blockquote elements `<blockquote>` respectively the additional CSS blockquote
class `.blockquote` available.

[source, html, role="mb-4"]
----

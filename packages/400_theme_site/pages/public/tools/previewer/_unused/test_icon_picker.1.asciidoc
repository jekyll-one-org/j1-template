...
<script>
  ...
  function init_select() {
    ...
    var $selectElement        = document.querySelector('select[name="icon-library"]');
  ...
  function cb_select_after_close() {
    logger.debug('\n' + 'selected item: ' + $selectElement.value);
    ...
    if (j1.getState() === 'finished' && pageVisible) {
      var dependencies_met_modules_ready = setInterval (() => {
        ...
        if (iconPickerFinished && slimSelectFinished) {
          logger.info('\n' + 'iconPicker: initializing page finished');

    ...
    if (j1.getState() === 'finished' && pageVisible) {
      ...
      var dependencies_met_modules_ready = setInterval( () => {
        ...
        if (iconPickerFinished && slimSelectFinished) {
          ...
          // set initial slimSelect values
          logger.info('\n' + 'slimSelect: set slimSelect initial values (page)');
          init_select();

          // set slimSelect events
          logger.info('\n' + 'slimSelect: set slimSelect events (page)');
          init_select_events()

          logger.info('\n' + 'slimSelect: initializing finished (page)');

...
Country flags can be used as inline icons by using
the `flag:` inline macro:
...
[source, prometheus, role="noclip"]
----
flag:de[rectangle, 6x, ml-3 mr-2 mb-2] Germany (rectangle) +
flag:de[squared, 6x, ml-3 mr-3 mb-2] Germany (square)
----

[.result]
====
flag:de[rectangle, 3x, ml-3 mr-2 mb-2] Germany (rectangle) +
flag:de[squared, 3x, ml-3 mr-3 mb-2] Germany (square)
====
...
.Example flag sizes (proportional)
[cols="2a,3a,4a,^", options="header", width="100%", role="rtable mt-3"]
|===
|Size |Style |Markup |Render

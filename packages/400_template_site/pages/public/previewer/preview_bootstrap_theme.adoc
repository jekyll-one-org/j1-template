...
several predefined button styles, each serving its semantic purpose, with a
...
FABs are adopted navigation elements from mobile devices (Android) but used
today quite often for web pages as well. Floating action buttons for the web
typically provide *additonal* actions for a page, e.g. in-page navigation.

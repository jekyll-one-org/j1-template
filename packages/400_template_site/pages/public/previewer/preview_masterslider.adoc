...
lite version does *not* support all features. Not all slider *layouts* and
...
=== Slider 1 -- Lightbox + Controls + Filters
...
this slider is using the (CSS) *filter* feature of Master Slider. Filters can
be used for example to *transform* the *style* of the images provided by a
slider. In this example, the slider images are transformed* from style *color*
to *sepia*.
...
|layout
|`boxed`
|The slider width and height is defined by the slider *options*
...
an MS Slider depends. Find the examples for *Asciidoc* and *Markdown*
below.
...
To use MS sliders for content sources of type *Markdown*, simply write
the *HTML placeholder* for a slider:
...
|layout
|`fullwidth`
|The slider spans the full width of the page
...
the slideshow. MS Slider provide functions to combine images and text
...
|controls
|`slideinfo`
|Details for *MSInfo* is configured slide-wise under key `slides`
...
*Slider 4* is using the following general settings:
...
|controls
|`bullets`
|Default settings. Configuration properties are defined in
`~/_data/defalt/masterslider.yml`
...
*Slider 5* is using the following general settings:
...
*Slider 6* is using the following general settings:
...
=== Slider 7 -- ThumbInfo + Lightbox (right|vertical)

*Slider 7* is using the following general settings:

...
* an Asciidoc Extension `textbook::` to integrate notebooks (in webpages)
...
Das Modul zur J1 NBI Integration steht als stabile Version zur Verfügung,
ist jedoch noch im Status *Experimentell*. Wesentlicher Grund für den
experimentellen Status ist das *Alter* der der zu Grunde liegenden Library
`nbinteract-core`.

NOTE: Aktuelle Updates der Library sind länger als ein Jahr her und die
...
Wie bein allen Modulen die mit J1 zur Verfügung stehen ist  *keine*
Programmierung erforderlich. Ein Adapter für J1 übernimmt die Integration,
...
*may* interfere with the HTML output of other pages of a J1 website. The
formatting done by MatJax may result in *strange outcomes* for other content.
...
Der Einsatz von J1 NBI erfordert eine gültige *Binder* configuration. Die
Binder configuration ist über ein öffentliches Repo bereitzustellen.
Ein Beispiel eines solchen Repos finden auf *Github* unter
https://github.com/jekyll-one/nbinteract-notebooks[Binder Repo, {browser-window--new}].
Die Einstellungen zur Konfiguration sind im Abschnitt <<Default settings>>
beschrieben.

=== Textbook configuration

Neben den Defaulteinstellungen für J1 NBI (`~/_data/default/nbinteract.yml`)
sind *individuelle* Einstellungen für jedes Textbook (nbinteract HTML Notebook)
erforderlich, dass in die Seiten eines J1 Webs eingebunden werden soll.
Beispiele dazu finden sie in der Konfigurationsdatei `~/_data/nbinteract.yml`,
eine Beschreibung der verwendeten Parameter im Abschnitt <<Notebook settings>>.

=== Textbook integration

Zur Integration von Textbooks (nbinteract HTML Notebook) in Webseiten ist das
J1 NBI Module a Resource einzubinden. Die `ID` des Moduls der J1 NBI Integration
ist: `nbinteract`.
...
Einbinden des Notebooks mit der ID `j1_docs_example_dynamic` in eine Webseite.
...
*interactivity*. Again, check the _Evaluation_ section at section
<<Dynamic Textbook>>.

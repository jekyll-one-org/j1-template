Prompt
  https://claude.ai/chat/1a994176-87f5-430d-890f-3b734eb32842
--------------------------------------------------------------------------------

``` 
 <span class="attic__badge_unsplash_icon">
   <svg xmlns="http://www.w3.org/2000/svg" class="attic__badge_unsplash_icon-size" viewBox="0 0 32 32">
    <path d="M10 9V0h12v9H10zm12 5h10v18H0V14h10v9h12v-9z"></path>
  </svg>
</span>
```

I need a similiar HTML element like span class="attic__badge_unsplash_icon"
for the attached ai sparkles icon (ai-sparkles.svg).


Claude
--------------------------------------------------------------------------------

Here's the same structure, with the Inkscape/SVG-Repo cruft stripped out (namedview, defs, empty `<g>` wrappers, `width`/`height`, and the pointless `stroke-width` style):

```html
<span class="attic__badge_ai_sparkles_icon">
  <svg xmlns="http://www.w3.org/2000/svg" class="attic__badge_ai_sparkles_icon-size" viewBox="0 0 5.6 5.6">
...
**Keep `viewBox="0 0 5.6 5.6"`**
The path coordinates are in that tiny user-unit space, so replacing it with
`0 0 32 32` to match the Unsplash icon would shrink the glyph to a speck
in the corner. The viewBox is what makes it scale; the CSS class controls
the rendered size.

**Colour** 
Your Unsplash SVG has no `fill`, so it paints with the SVG default (black)
unless a CSS rule targets it. The source file here had a hardcoded
`fill="#000000"` on the root, which I dropped so both icons behave
identically. If you want the icon to follow the surrounding text colour,
add `fill="currentColor"` to the `<svg>` element and it'll pick up
whatever `color` the badge inherits.

If you'd rather not add a new size class, just reuse
`attic__badge_unsplash_icon-size` — the viewBox difference doesn't matter
as long as the CSS sets `width`/`height` rather than relying on intrinsic
dimensions.
